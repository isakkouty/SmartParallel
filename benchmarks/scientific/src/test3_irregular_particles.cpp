...
namespace
{
    ...
    double evaluate_particle(const Particle& particle, std::size_t index)
    {
        ...
        for (std::uint32_t k = 0; k < particle.work_units; ++k)
        {
            ...
            energy += particle.mass * (std::sin(radius2 * 0.001) + std::cos(phase)) /
                std::sqrt(radius2);
        }
        return energy;
    }
    ...
    {
        smart::parallel_for(std::size_t{0}, particles.size(), [&](std::size_t i)
        {
            output[i] = evaluate_particle(particles[i], i);
        });
    }
    ...
    const char* strategy_name(smart::ExecutionStrategy strategy)
    {
        switch (strategy)
        {
        case smart::ExecutionStrategy::Sequential: return "Sequential";
        case smart::ExecutionStrategy::StaticChunks: return "StaticChunks";
        case smart::ExecutionStrategy::DynamicChunks: return "DynamicChunks";
    ...
    void write_csv(const std::filesystem::path& path, const std::vector<Result>& results)
    {
        ...
        {
            output << result.case_name << ',' << result.particles << ','
                   << result.total_work_units << ',' << result.min_work_units << ','
                   << result.max_work_units << ',' << result.sequential_ms << ','
                   << result.smartparallel_ms << ',' << result.speedup << ','
                   << result.max_error << ',' << result.sequential_checksum << ','
                   << result.smartparallel_checksum << ',' << result.engine << ','
                   << result.strategy << ',' << result.workers << ',' << result.chunk_size << ','
...
int main(int argc, char** argv)
{
    try
    {
        const std::filesystem::path output_path = argc > 1
            ...
            : std::filesystem::path("validation/output/scientific_test3_irregular_particles.csv");
        ...
        std::cout << "Kernel: deterministic particle-energy evaluation with 16..511 work units per particle\n";
        std::cout << "Purpose: exercise adaptive scheduling under strongly non-uniform iteration cost\n\n";
        ...
        for (const Case& test_case : cases)
        {
            ...
            std::cout << std::left << std::setw(20) << test_case.name
                      << " particles=" << test_case.particles
                      << " work=" << total_work_units
                      << " plan=" << result.engine << '/' << result.strategy
                      << "/w" << result.workers << "/c" << result.chunk_size
