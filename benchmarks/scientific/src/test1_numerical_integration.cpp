...
namespace
{
    ...
    {
        smart::parallel_for(std::size_t{0}, contributions.size(), [&](std::size_t i)
        {
            const double x = integration_begin + (static_cast<double>(i) + 0.5) * step;
            contributions[i] = integrand(x) * step;
        });
    ...
    void write_csv(const std::filesystem::path& path, const std::vector<Result>& results)
    {
        ...
        {
            output << result.case_name << ',' << result.intervals << ','
                   << result.sequential_ms << ',' << result.smartparallel_ms << ','
                   << result.speedup << ',' << result.sequential_result << ','
                   << result.smartparallel_result << ',' << result.analytical_result << ','
                   << result.absolute_error << ',' << result.discretization_error << ','
                   << result.engine << ',' << result.strategy << ',' << result.workers << ','
                   << result.chunk_size << ',' << (result.cache_hit ? "true" : "false") << ','
...
int main(int argc, char** argv)
{
    try
    {
        const std::filesystem::path output_path = argc > 1
            ...
            : std::filesystem::path("validation/output/scientific_test1_numerical_integration.csv");
        ...
        {
            const double step = (integration_end - integration_begin) /
                static_cast<double>(test_case.intervals);
