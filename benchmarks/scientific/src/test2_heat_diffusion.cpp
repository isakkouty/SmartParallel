...
namespace
{
                                ...
                                std::size_t height)
    {
        for (std::size_t y = 1; y + 1 < height; ++y)
        {
            ...
            for (std::size_t x = 1; x + 1 < width; ++x)
            {
                ...
                output[i] = center + alpha *
                    (input[i - 1] + input[i + 1] + input[i - width] + input[i + width] - 4.0 * center);
            }
        }
    }
    ...
    void update_rows_smartparallel(const std::vector<double>& input,
                                   ...
                                   std::size_t height)
    {
        smart::parallel_for(std::size_t{1}, height - 1, [&](std::size_t y)
        {
            const std::size_t row = y * width;
            for (std::size_t x = 1; x + 1 < width; ++x)
            {
...
int main(int argc, char** argv)
{
    try
    {
        ...
        {
            const std::vector<double> initial = make_initial_field(test_case.width, test_case.height);
            ...
            {
                simulate_sequential(initial, sequential_state, sequential_scratch,
                    test_case.width, test_case.height, test_case.time_steps);
            });
            const double smartparallel_ms = median_runtime_ms(test_case.repetitions, [&]
            {
                simulate_smartparallel(initial, smart_state, smart_scratch,
                    test_case.width, test_case.height, test_case.time_steps);
            });

            // Recompute once after timing so both result buffers unambiguously contain the final field.
            simulate_sequential(initial, sequential_state, sequential_scratch,
                test_case.width, test_case.height, test_case.time_steps);
            simulate_smartparallel(initial, smart_state, smart_scratch,
                test_case.width, test_case.height, test_case.time_steps);
            ...
            std::cout << std::left << std::setw(20) << test_case.name
                      << " grid=" << test_case.width << 'x' << test_case.height
                      << " steps=" << test_case.time_steps
