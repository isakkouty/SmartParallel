...
std::string selected_plan()
{
    ...
    return std::string(smart::engine_name(plan.engine)) + "/" +
        smart::execution_strategy_name(plan.strategy) + "/w" +
...
{
    cv::parallel_for_(
        cv::Range(0, static_cast<int>(count)),
...
inline std::uint32_t procedural_pixel(
    std::uint32_t x,
    std::uint32_t y,
    std::uint32_t frame)
{
    Value value =
        static_cast<Value>(x) * 0x9e3779b185ebca87ULL ^
    ...
    Task&& task)
{
    ...
    const double sequential_ms = median_runtime_ms(
        repetitions,
        [&]
        {
    ...
    const double opencv_parallel_ms = median_runtime_ms(
        repetitions,
        [&]
        {
            opencv_for(
                ...
                [&](std::size_t i)
                {
                    opencv_parallel[i] = task(i);
                });
        });

    const double smartparallel_ms = median_runtime_ms(
        repetitions,
        [&]
        {
            smart_for(
    ...
    const bool correct =
        sequential == opencv_parallel &&
        sequential == smartparallel;
    ...
    return run_benchmark(
        ...
        [](std::size_t task_index) -> Value
        {
            const std::uint32_t tile_x =
                static_cast<std::uint32_t>(task_index & 255U);
            const std::uint32_t tile_y =
                static_cast<std::uint32_t>((task_index >> 8U) & 255U);
            ...
            for (int py = 0; py < 8; ++py)
            {
                ...
                {
                    const double x =
                        (static_cast<double>(tile_x * 8 + px) /
                         2048.0) * 3.2 - 2.2;
                    const double y =
                        (static_cast<double>(tile_y * 8 + py) /
                         2048.0) * 2.4 - 1.2;
                    ...
                    int iteration = 0;
                    const int limit =
                        48 + static_cast<int>((tile_x + tile_y) & 31U);
    ...
    return run_benchmark(
        ...
        [](std::size_t task_index) -> Value
        {
            ...
            const std::uint32_t frame =
                static_cast<std::uint32_t>(task_index >> 12U);
            const std::uint32_t local =
                static_cast<std::uint32_t>(task_index & 4095U);
            ...
            while (top > 0)
            {
                ...
                const std::uint32_t p0 =
                    procedural_pixel(node.x, node.y, frame);
                ...
                const std::uint32_t minimum =
                    std::min(std::min(p0, p1), std::min(p2, p3));
                    ...
                    contrast > 54U)
                {
                    ...
                    stack[top++] =
                        {node.x, node.y, half, node.depth + 1U};
                    stack[top++] =
                        {node.x + half, node.y, half, node.depth + 1U};
                    stack[top++] =
                        {node.x, node.y + half, half, node.depth + 1U};
                    stack[top++] =
                        {node.x + half,
                         node.y + half,
                         half,
...
Result monte_carlo_sampling_stress()
{
    ...
    return run_benchmark(
        ...
        {
            Value state =
                mix(static_cast<Value>(task_index) +
                 ...
                 ++sample)
            {
                ...
                inside += radius <
                    static_cast<std::int64_t>(32768) * 32768;

                luminance +=
                    procedural_pixel(
                        static_cast<std::uint32_t>(x + 32768),
                        static_cast<std::uint32_t>(y + 32768),
                        static_cast<std::uint32_t>(task_index));
            }

            return mix(
                static_cast<Value>(inside) ^
                (luminance << 16U) ^
                state);
        });
}
    ...
    return run_benchmark(
        ...
        [](std::size_t task_index) -> Value
        {
            const std::uint32_t x =
                static_cast<std::uint32_t>(task_index & 2047U);
            const std::uint32_t y =
                static_cast<std::uint32_t>(
                    (task_index >> 11U) & 2047U);
            const std::uint32_t frame =
                static_cast<std::uint32_t>(task_index >> 22U);

            Value checksum = 0;
            int active = 0;

            for (int repeat = 0;
                 repeat < 2 + static_cast<int>(task_index & 3U);
                 ++repeat)
            {
                ...
                for (int dy = -3; dy <= 3; ++dy)
                {
                    for (int dx = -3; dx <= 3; ++dx)
                    {
                        const std::uint32_t value =
                            procedural_pixel(
                                x + static_cast<std::uint32_t>(dx + 3),
    ...
    return run_benchmark(
        ...
        [](std::size_t task_index) -> Value
        {
            ...
            const std::uint32_t base_y =
                static_cast<std::uint32_t>(
                    ((task_index >> 10U) & 1023U) * 8U);
            const std::uint32_t frame =
                static_cast<std::uint32_t>(task_index >> 20U);
            ...
            for (int y = 0; y < 10; ++y)
            {
                for (int x = 0; x < 10; ++x)
                {
                    patch[y][x] =
                        procedural_pixel(
                            base_x + static_cast<std::uint32_t>(x),
    ...
    return run_benchmark(
        ...
        [](std::size_t task_index) -> Value
        {
            ...
            for (int repeat = 0; repeat < repeats; ++repeat)
            {
                const std::uint32_t x =
                    static_cast<std::uint32_t>(
                        task_index + repeat * 17U);
                const std::uint32_t y =
                    static_cast<std::uint32_t>(
                        task_index * 3U + repeat * 29U);
                ...
                case 1:
                {
                    std::uint32_t sum = 0;
                    for (int k = 0; k < 25; ++k)
                        sum += procedural_pixel(
                            x + static_cast<std::uint32_t>(k),
                ...
                case 3:
                {
                    ...
                    for (int k = 0; k < 49; ++k)
                    {
                        const std::uint32_t value =
                            procedural_pixel(
                                x + static_cast<std::uint32_t>(k % 7),
    ...
    const std::vector<Result>& results)
{
    ...
    if (!out)
        throw std::runtime_error(
            "Could not open output CSV: " + path.string());

    out <<
        "benchmark,work_items,sequential_ms,"
    ...
    for (const auto& result : results)
    {
        out <<
            ...
            result.opencv_parallel_ms << ',' <<
            result.smartparallel_ms << ',' <<
...
int main(int argc, char** argv)
{
    try
    {
        ...
        std::cout <<
            "==== SmartParallel OpenCV scheduler stress suite ====\n"
        ...
        for (const auto& result : results)
        {
            ...
            std::cout <<
                std::left <<
                std::setw(32) << result.benchmark <<
                " work=" <<
                std::setw(9) << result.work_items <<
                " smart=" <<
                std::setw(36) << result.smart_plan <<
                " correct=" <<
