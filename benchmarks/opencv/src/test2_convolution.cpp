...
namespace
{
    ...
    void convolve_linear_range(const cv::Mat& padded, cv::Mat& destination,
    ...
    void convolution_opencv_parallel(const cv::Mat& padded, cv::Mat& destination)
    {
        ...
        {
            convolve_linear_range(padded, destination,
    ...
    void convolution_smartparallel(const cv::Mat& padded, cv::Mat& destination)
    {
        ...
        smart::parallel_for(0, total, [&](std::size_t index)
        {
    ...
    std::string selected_plan()
    {
        ...
        return std::string(smart::engine_name(plan.engine)) + "/" +
            ...
            std::to_string(plan.job_count) + "/c" + std::to_string(plan.chunk_size);
    }
    ...
    cv::Mat opencv_kernel()
    {
        ...
        for (int y = 0; y < kernel_size; ++y)
            for (int x = 0; x < kernel_size; ++x)
                matrix.at<float>(y, x) = kernel[static_cast<std::size_t>(y * kernel_size + x)] * kernel_scale;
        return matrix;
    }
...
int main(int argc, char** argv)
{
    try
    {
        ...
        for (const Case& test_case : cases)
        {
            ...
            cv::Mat padded;
            cv::copyMakeBorder(source, padded, kernel_radius, kernel_radius, kernel_radius, kernel_radius,
                               cv::BORDER_REPLICATE);
            ...
            const double opencv_filter_ms = median_runtime_ms(test_case.repetitions, [&]
            {
                cv::filter2D(source, opencv_reference, CV_32F, filter_kernel, cv::Point(-1, -1), 0.0,
                             cv::BORDER_REPLICATE);
            });

            const double manual_difference = max_absolute_difference(sequential, smartparallel);
            const double opencv_parallel_difference = max_absolute_difference(sequential, opencv_parallel);
            const double opencv_difference = max_absolute_difference(sequential, opencv_reference);
            const bool correct = manual_difference <= 1.0e-5 &&
                opencv_parallel_difference <= 1.0e-5 && opencv_difference <= 2.0e-4;
            all_correct = all_correct && correct;

            results.push_back(Result{test_case.name, test_case.width, test_case.height, source.total(),
                                     sequential_ms, opencv_parallel_ms, smartparallel_ms, opencv_filter_ms,
                                     plan, std::max(manual_difference, opencv_parallel_difference),
                                     opencv_difference, correct});
