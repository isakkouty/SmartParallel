...
void raw_threaded_for(std::vector<int>& values)
{
         ...
         ++t)
    {
        const std::size_t begin =
            (total * t) / thread_count;

        const std::size_t end =
            (total * (t + 1)) / thread_count;
...
volatile long long memory_sink = 0;
...
Result run_case(std::size_t count)
{
    ...
    Result result;
    result.size = count;
    result.bytes_processed = count * sizeof(int) * 2;
    ...
    for (int run = 0; run < runs; ++run)
{
    ...
    smart::for_each(values, memory_work);
...
int main()
{
    ...
    std::ofstream csv(
        "benchmarks\\memory_bandwidth\\output\\beta_1_0\\results.csv"
    );
