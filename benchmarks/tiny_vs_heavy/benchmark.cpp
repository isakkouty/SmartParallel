...
void consume_values(const std::vector<int>& values)
{
    long long sum = 0;
...
void tiny_work(int& value)
{
    value += 1;
}

void heavy_work(int& value)
{
...
void raw_threaded_for(std::vector<int>& values, Function work)
{
    ...
    for (std::size_t t = 0; t < thread_count; ++t)
    {
        ...
        threads.emplace_back([begin, end, &values, work]()
        {
...
{
    std::cout << std::setw(6) << result.function_name
...
int main()
{
    ...
    std::ofstream csv(
        "benchmarks\\tiny_vs_heavy\\output\\beta_1_0\\results.csv"
    );

    write_csv_header(csv);

    std::cout << "==== Tiny vs Heavy Benchmark ====\n";
    ...
    {
        Result tiny = run_case("tiny", size, tiny_work);
        write_csv_row(csv, tiny);
        print_result(tiny);

        Result heavy = run_case("heavy", size, heavy_work);
        write_csv_row(csv, heavy);
        print_result(heavy);
    }

    std::cout << "\nResults written to:\n";
    std::cout << "benchmarks\\tiny_vs_heavy\\output\\beta_1_0\\results.csv\n";
