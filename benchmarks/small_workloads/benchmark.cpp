...
SmartTiming read_smart_timing()
{
    ...
    for (const smart::TimingPhase& phase : smart::last_timing_report().phases)
    {
        ...
        {
            timing.execution_ms += phase.elapsed_ms;
        }
        else
        {
...
Result run_case(std::size_t count)
{
    ...
    for (int run = 0; run < runs; ++run)
    {
        ...
        smart::for_each(values, small_work);
...
int main()
{
    ...
    std::ofstream csv(
        "benchmarks\\small_workloads\\output\\beta_1_0\\results.csv"
    );

    write_csv_header(csv);

    std::cout << "==== Small Workloads Benchmark ====\n";
    ...
    std::cout << "\nResults written to:\n";
    std::cout << "benchmarks\\small_workloads\\output\\beta_1_0\\results.csv\n";
