...
set "BUILD_DIR=build_v1_opencv_test2"
...
echo.
echo ==== [1/3] Configure OpenCV Test 2 ====
...
cmake -S . -B "%BUILD_DIR%" ^
  ...
  -DSMARTPARALLEL_BUILD_OPENCV_TEST2=ON
if errorlevel 1 exit /b 1

echo.
echo ==== [2/3] Build OpenCV Test 2 ====
cmake --build "%BUILD_DIR%" --target smartparallel_opencv_test2
if errorlevel 1 exit /b 1

echo.
echo ==== [3/3] Run OpenCV Test 2 ====
if not exist validation\output mkdir validation\output
"%BUILD_DIR%\smartparallel_opencv_test2.exe" "validation\output\opencv_test2_convolution.csv"
if errorlevel 1 exit /b 1

echo.
echo ============================================================
echo OpenCV Test 2 completed.
echo Results: validation\output\opencv_test2_convolution.csv
echo ============================================================
