...
namespace smart::ranking
{
    ...
    inline std::vector<double> make_runtime_raw_features(
        ...
        const ExecutionPlan& plan)
    {
        ...
        std::vector<double> values = {
            ...
            std::log1p(std::max(0.0, profile_available ? profile->median_ms_per_iteration : 0.0) * 1.0e6),
        ...
        for (double flag : backend_flags) values.push_back(flag);
        for (const char* name : schedules) values.push_back(schedule == name ? 1.0 : 0.0);
