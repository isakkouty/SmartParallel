...
namespace smart::ranking
{
    struct UtilityModelArtifact
    {
        ...
        bool promoted() const noexcept { return promotion_status == "PROMOTED"; }
        ...
        void validate() const
        {
            ...
            for (double value : scaler_means)
                if (!std::isfinite(value)) throw std::runtime_error("non-finite scaler mean");
            for (double value : scaler_scales)
                if (!std::isfinite(value) || value <= 0.0) throw std::runtime_error("invalid scaler scale");
            for (double value : model.weights())
                if (!std::isfinite(value)) throw std::runtime_error("non-finite utility-model weight");
        }
    ...
    namespace detail
    {
        ...
        {
            std::size_t count = 0;
            if (!(input >> count)) throw std::runtime_error("missing count for " + field);
            std::vector<double> values(count);
            for (double& value : values)
                if (!(input >> value)) throw std::runtime_error("invalid value in " + field);
            return values;
        }
    }

    inline void save_utility_model_artifact(const UtilityModelArtifact& artifact, const std::string& path)
    {
        artifact.validate();
        std::ofstream output(path, std::ios::binary | std::ios::trunc);
        if (!output) throw std::runtime_error("cannot open utility-model artifact for writing: " + path);
        ...
        output << "END\n";
        if (!output) throw std::runtime_error("failed while writing utility-model artifact: " + path);
    }

    inline UtilityModelArtifact load_utility_model_artifact(const std::string& path)
    {
        std::ifstream input(path, std::ios::binary);
        if (!input) throw std::runtime_error("cannot open utility-model artifact: " + path);
        ...
        while (std::getline(input, line))
        {
            ...
            if (key == "feature_schema") row >> artifact.feature_schema;
            else if (key == "promotion_status") row >> artifact.promotion_status;
            else if (key == "hardware_fingerprint") row >> artifact.hardware_fingerprint;
            else if (key == "scaler_means") artifact.scaler_means = detail::read_vector(row, key);
            else if (key == "scaler_scales") artifact.scaler_scales = detail::read_vector(row, key);
            else if (key == "weights") weights = detail::read_vector(row, key);
            else if (key == "END") { end_seen = true; break; }
            else throw std::runtime_error("unknown utility-model field: " + key);
        }
        if (!header_seen || !end_seen) throw std::runtime_error("incomplete SmartParallel utility-model artifact");
