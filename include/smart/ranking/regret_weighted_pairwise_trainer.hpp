...
namespace smart::ranking
{
    ...
    public:
        ...
        explicit RegretWeightedPairwiseTrainer(Options options) : options_(options) {}
        ...
        double train_one(LinearUtilityModel& model, const PairwiseTrainingExample& example) const
        {
            ...
            const double margin_error = model.score(example.better_features) -
                model.score(example.worse_features);
            ...
            {
                const double feature_difference = example.better_features[i] - example.worse_features[i];
                const double gradient = importance * probability * feature_difference + options_.l2 * parameters[i];
