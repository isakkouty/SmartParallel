...
class HistoricalDecisionProvider
{
  public:
                                         ...
                                         const ExecutionHints& hints) const
    {
        ...
        WorkloadFingerprint fp = fingerprint(context.workload, context.function_profile);

        const ExperienceEntry* entry = global_experience_database().best_entry(fp);
        ...
        DecisionReport report = analytical.decide(context, hints).value();
