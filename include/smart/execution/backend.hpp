...
namespace smart
{
    ...
    public:
            ...
            std::function<void(std::size_t)> func) override
        {
            ...
            const std::size_t workers = std::max<std::size_t>(
                1,
                std::min({job_count, total, pool.thread_count()}));
            ...
            {
                pool.submit([total, grain, &next, func]()
                {
                    while (true)
                    {
    ...
    public:
            ...
            std::function<void(std::size_t)> func) override
        {
            ...
            const std::size_t workers = std::max<std::size_t>(
                1,
                std::min(total, job_count));
            const std::size_t grain = std::max<std::size_t>(
                1,
                chunk_size == 0 ? 1 : chunk_size);
            ...
            {
                tbb::parallel_for(
                    tbb::blocked_range<std::size_t>(0, total, grain),
            ...
            std::function<void(std::size_t)> func) override
        {
            ...
            for (std::size_t t = 0; t < job_count; ++t)
            {
                ...
                const std::size_t begin =
                    t * base + std::min(t, remainder);
                const std::size_t end =
                    begin + base + (t < remainder ? 1 : 0);
