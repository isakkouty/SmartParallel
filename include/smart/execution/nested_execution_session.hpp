...
inline void append_nested_trace_record(NestedExecutionTraceRecord record)
{
    ...
    const std::size_t maximum = global_config().nested_execution_trace_max_records;
    if (maximum != 0)
    {
    ...
    const std::vector<NestedExecutionTraceRecord>& records = nested_execution_trace_snapshot())
{
    ...
    out << "root_loop_id,loop_id,parent_loop_id,callsite_hash,parent_callsite_hash,depth,iterations,"
           "phase,backend,policy,mechanism,decision_reason,cache_hit,profile_available,parallel,"
    ...
    for (const auto& r : records)
    {
        ...
        out << r.root_loop_id << ',' << r.loop_id << ',' << r.parent_loop_id << ','
            ...
            << r.iterations << ',' << safe(r.phase) << ',' << safe(r.backend) << ','
...
class NestedExecutionSession : public std::enable_shared_from_this<NestedExecutionSession>
{
  public:
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        const std::size_t maximum = global_config().nested_plan_snapshot_max_entries;
        if (maximum != 0 && plan_snapshots_.size() >= maximum
    ...
    {
        if (!global_config().enable_nested_execution_trace)
            return;
        std::lock_guard<std::mutex> lock(mutex_);
