...
namespace smart
{
    ...
    namespace detail
    {
        ...
        inline TimingObservation measure_plan_total(
            ...
            Function function)
        {
            ...
            result.relative_mad = result.median_ms > 0.0
                ? std::clamp(1.4826 * mad / result.median_ms, 0.0, 2.0)
                : 1.0;
            return result;
        }
        ...
        inline std::size_t chunk_for_count(
            std::size_t iterations,
            std::size_t chunks)
        {
            return std::max<std::size_t>(
                1,
                (iterations + std::max<std::size_t>(1, chunks) - 1) /
                    std::max<std::size_t>(1, chunks));
        }

        inline double overhead_from_curve(
            double base,
            double slope,
            std::size_t total_chunks)
        {
            return std::max(
                0.0,
                base + slope * std::log2(
                    static_cast<double>(std::max<std::size_t>(1, total_chunks)) +
                    1.0));
        }

        inline double useful_speedup(
            ...
            std::size_t workers)
        {
            if (!std::isfinite(sequential_ms) ||
                !std::isfinite(parallel_total_ms) ||
                sequential_ms <= 0.0 || parallel_total_ms <= 0.0)
            {
                return 1.0;
            }
            ...
            const double removable_overhead = std::min(
                std::max(0.0, scheduler_overhead_ms),
                parallel_total_ms * 0.25);
            const double ideal_floor = sequential_ms /
                std::max(1.0, static_cast<double>(workers));
            const double parallel_useful_ms = std::max(
                ideal_floor,
                parallel_total_ms - removable_overhead);
            return std::clamp(
                sequential_ms / parallel_useful_ms,
                1.0,
                std::max(1.0, static_cast<double>(workers)));
        }
        ...
        template <typename Function>
        inline SpeedupObservation measure_useful_speedup(
            ExecutionEngineType engine,
            ExecutionStrategy strategy,
            std::size_t workers,
            std::size_t iterations,
            ...
            Function function)
        {
            ...
            const std::size_t desired_chunks = std::max<std::size_t>(
                workers * 8,
                workers);
            const std::size_t chunk = chunk_for_count(
                iterations,
                desired_chunks);
            ...
            const TimingObservation sequential_timing = measure_plan_total(
                sequential, iterations, runs, function);
            const TimingObservation parallel_timing = measure_plan_total(
                parallel, iterations, runs, function);
            const std::size_t actual_chunks = strategy ==
                    ExecutionStrategy::DynamicChunks
                ? (iterations + chunk - 1) / chunk
                : workers;
            const double overhead = overhead_from_curve(
                overhead_base,
                overhead_slope,
                actual_chunks);
            const double speedup = useful_speedup(
                sequential_timing.median_ms,
                parallel_timing.median_ms,
                overhead,
                workers);
            const double uncertainty = std::clamp(
                std::hypot(
                    sequential_timing.relative_mad,
                    parallel_timing.relative_mad),
                0.0,
                2.0);

            const double signal_ms = std::min(
                sequential_timing.median_ms,
                parallel_timing.median_ms);
            const double duration_confidence = smoothstep(
                0.25, 5.0, signal_ms);
            const double overhead_fraction = parallel_timing.median_ms > 0.0
                ? std::clamp(
                    overhead / parallel_timing.median_ms, 0.0, 1.0)
                : 1.0;
            const double overhead_confidence = 1.0 - smoothstep(
                0.15, 0.55, overhead_fraction);
            const double stability_confidence = std::clamp(
                1.0 / (1.0 + 4.0 * uncertainty), 0.0, 1.0);
            ...
            observation.confidence = std::clamp(
                duration_confidence * overhead_confidence *
            ...
            std::size_t workers)
        {
            ...
            const TimingObservation sequential_scheduler = measure_plan_total(
                sequential,
                overhead_iterations,
                overhead_runs,
                scheduler_function);
            ...
            {
                const std::size_t low_chunk_count = std::max<std::size_t>(
                    workers * 2,
                    2);
                const std::size_t high_chunk_count = std::max<std::size_t>(
                    workers * 32,
                    low_chunk_count + 1);
                const std::size_t low_chunk = chunk_for_count(
                    overhead_iterations,
                    low_chunk_count);
                const std::size_t high_chunk = chunk_for_count(
                    overhead_iterations,
                    high_chunk_count);
                const TimingObservation low = measure_plan_total(
                    calibration_plan(engine, strategy, workers, low_chunk),
                    overhead_iterations,
                    overhead_runs,
                    scheduler_function);
                const TimingObservation high = measure_plan_total(
                    calibration_plan(engine, strategy, workers, high_chunk),
                    overhead_iterations,
                    overhead_runs,
                    scheduler_function);

                const double ideal_sequential =
                    sequential_scheduler.median_ms /
                    static_cast<double>(workers);
                const double low_overhead = std::max(
                    0.0,
                    low.median_ms - ideal_sequential);
                const double high_overhead = std::max(
                    low_overhead,
                    high.median_ms - ideal_sequential);
                const double low_log = std::log2(
                    static_cast<double>(low_chunk_count) + 1.0);
                const double high_log = std::log2(
                    static_cast<double>(high_chunk_count) + 1.0);
                overhead_slope = std::max(
                    0.0,
                    (high_overhead - low_overhead) /
                        std::max(1.0e-9, high_log - low_log));
                overhead_base = std::max(
                    0.0,
                    low_overhead - overhead_slope * low_log);
                overhead_uncertainty = std::clamp(
                    std::max({
                        sequential_scheduler.relative_mad,
                        low.relative_mad,
                        high.relative_mad}),
                    0.0,
                    2.0);
            ...
            {
                const TimingObservation parallel_scheduler = measure_plan_total(
                    calibration_plan(engine, strategy, workers, 0),
                    overhead_iterations,
                    overhead_runs,
                    scheduler_function);
                overhead_base = std::max(
                ...
                overhead_uncertainty = std::clamp(
                    std::max(
                        sequential_scheduler.relative_mad,
                        parallel_scheduler.relative_mad),
                    0.0,
                    2.0);
            }

            std::vector<std::uint64_t> cheap_values(
            ...
            const auto cheap_small = measure_useful_speedup(
                engine,
                strategy,
                workers,
                ...
                overhead_slope,
                cheap_function);
            const auto cheap_large = measure_useful_speedup(
                engine,
                strategy,
                workers,
                cheap_large_iterations,
                ...
                overhead_slope,
                cheap_function);
            runtime_calibration_sink ^=
                cheap_values[cheap_values.size() / 2];

            std::vector<std::uint64_t> compute_values(
                compute_iterations,
                0u);
            ...
            const auto compute = measure_useful_speedup(
                engine,
                strategy,
                workers,
                compute_iterations,
                ...
                compute_function);
            runtime_calibration_sink ^=
                compute_values[compute_values.size() / 2];

            std::vector<std::uint64_t> stream_values(
                stream_iterations,
                1u);
            ...
            const auto stream = measure_useful_speedup(
                engine,
                strategy,
                workers,
                stream_iterations,
                ...
                stream_function);
            runtime_calibration_sink ^=
                stream_values[stream_values.size() / 2];
            ...
            point.speedup_relative_uncertainty = std::clamp(
                std::max({
                    cheap_small.relative_uncertainty,
            ...
            double weight)
        {
            ...
            return std::exp(
                (1.0 - t) * std::log(safe_left) +
                t * std::log(safe_right));
        }
    }
        ...
        std::size_t chunk_size)
    {
        ...
        return detail::overhead_from_curve(
        ...
        const RuntimeCalibrationQuery& query)
    {
        ...
        const double size_position = detail::smoothstep(
            ...
            std::log2(static_cast<double>(
                std::max<std::size_t>(1, query.iterations))));
        ...
        const double cost_us = std::max(
            1.0e-6,
            query.per_iteration_ms * 1.0e3);
        const double compute_weight = detail::smoothstep(
            std::log2(0.08),
            std::log2(2.0),
            std::log2(cost_us));
        const double compute_like = detail::geometric_blend(
            cheap_speedup,
            point->compute_speedup,
            compute_weight);
        return std::clamp(
            detail::geometric_blend(
                compute_like,
    ...
    inline double calibrated_backend_speedup_authority(
        ExecutionEngineType engine,
        std::size_t workers,
        const RuntimeCalibrationQuery& query)
    {
        const BackendRuntimeCalibrationPoint* point = detail::closest_point(
            backend_runtime_calibration(), engine, workers);
        if (point == nullptr)
            return 0.0;
        ...
        const double size_position = detail::smoothstep(
            ...
            std::log2(static_cast<double>(
                std::max<std::size_t>(1, query.iterations))));
        const double cheap_confidence =
            (1.0 - size_position) * point->cheap_small_confidence +
            size_position * point->cheap_large_confidence;

        const double cost_us = std::max(
            1.0e-6, query.per_iteration_ms * 1.0e3);
        const double compute_weight = detail::smoothstep(
            std::log2(0.08),
            std::log2(2.0),
            std::log2(cost_us));
        const double compute_like_confidence =
            (1.0 - compute_weight) * cheap_confidence +
            compute_weight * point->compute_confidence;
        const double streaming = std::clamp(
            query.streaming_strength, 0.0, 1.0);
        const double probe_confidence =
            (1.0 - streaming) * compute_like_confidence +
            streaming * point->streaming_confidence;
        ...
        // crossover.
        const double out_of_domain = std::clamp(
            std::max({
        ...
        const double domain_scale =
            (1.0 - out_of_domain) +
            out_of_domain *
                global_config().machine_calibration_out_of_domain_scale;
        ...
        RuntimeWorkloadClass workload_class)
    {
        ...
        query.iterations = workload_class == RuntimeWorkloadClass::ComputeLike
            ? 32'768
            : 1'048'576;
        query.per_iteration_ms =
            workload_class == RuntimeWorkloadClass::ComputeLike
                ? 0.002
                : 0.00005;
        query.streaming_strength =
            workload_class == RuntimeWorkloadClass::StreamingLike
                ? 1.0
                : workload_class == RuntimeWorkloadClass::General
                    ? 0.35
                    : 0.0;
        return calibrated_backend_speedup(engine, workers, query);
    }

    inline double calibrated_backend_relative_uncertainty(
        ExecutionEngineType engine,
        std::size_t workers)
    {
        const BackendRuntimeCalibrationPoint* point = detail::closest_point(
