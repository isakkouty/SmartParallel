...
namespace smart
{
            ...
            const WorkloadFingerprint& fingerprint) const
        {
            ...
            for (std::size_t index = 0; index < candidates.size(); ++index)
            {
                ...
                if (!candidate.available ||
                    !positive(candidate.predicted_total_ms))
                {
                    continue;
                }

                const double learned_sigma = std::max(
                    0.0, candidate.predicted_runtime_stddev_ms);
                const double base_sigma = candidate.predicted_total_ms *
                    0.05 * (1.0 - std::clamp(candidate.confidence, 0.0, 1.0));
                const double calibration_sigma =
                    candidate.machine_calibration_used
                        ? candidate.predicted_total_ms *
                            config.machine_calibration_risk_scale *
                            std::clamp(
                                candidate.machine_calibration_relative_uncertainty,
                                0.0,
                                1.0)
                        : 0.0;
                const double sigma = learned_sigma + base_sigma +
                    calibration_sigma;
                candidate.risk_adjusted_total_ms =
                    config.enable_risk_aware_ranking
                        ? candidate.predicted_total_ms +
                            config.runtime_uncertainty_risk_weight * sigma
                        : candidate.predicted_total_ms;
                analytical_best = std::min(
                    analytical_best,
                    candidate.risk_adjusted_total_ms);

                const bool has_explicit_baseline =
                    positive(candidate.analytical_baseline_total_ms);
                explicit_stable_baseline =
                    explicit_stable_baseline || has_explicit_baseline;
            ...
            for (PlanCostEstimate& candidate : candidates)
            {
                ...
                candidate.analytical_rank_score =
                    candidate.risk_adjusted_total_ms / analytical_best;
                ...
                const double transfer_weight = std::min(
                    candidate.similarity_rank_used
                        ? config.maximum_similarity_history_weight *
                ...
                const double transfer_score = candidate.similarity_rank_used
                    ...
                    candidate.similarity_rank_used)
                {
                    candidate.confidence = std::clamp(
                        candidate.confidence +
                            0.15 * exact_weight +
                            0.05 * transfer_weight,
                        0.0,
                        1.0);
                }
            }

            if (config.enable_learned_override_guard &&
                explicit_stable_baseline &&
    ...
    private:
        static void apply_exact_history(
            PlanCostEstimate& candidate,
            ...
            const Config& config)
        {
            const ExperienceEntry* entry =
                global_experience_database().find_plan(
                    ...
                    candidate.plan);
            if (entry == nullptr ||
                entry->sample_count < config.minimum_ranking_samples ||
            ...
            const double regret = std::max(
                0.0, entry->decayed_regret_percent);
            ...
            double exact_score = config.enable_simplified_historical_ranking
                ? 1.0 + regret / 100.0 +
                    0.10 * uncertainty
                : 1.0 +
                    config.ranking_regret_penalty_scale * regret / 100.0 +
                    config.ranking_uncertainty_penalty_scale * uncertainty +
                    (1.0 - std::clamp(
                        entry->decayed_success_rate, 0.0, 1.0)) * 0.20;
            ...
            const double full_confidence_samples = static_cast<double>(
                std::max<std::size_t>(
                    1, config.ranking_full_confidence_samples));
            const double evidence_confidence = std::clamp(
                1.0 - std::exp(
                    -effective_evidence /
                        full_confidence_samples * 3.0),
                0.0,
                1.0);
            ...
            const double stability_confidence = std::clamp(
                1.0 /
                    (1.0 + config.ranking_stability_scale *
                        coefficient_of_variation),
                0.0,
                1.0);
            const double prediction_error_scale = std::max(
                1.0, config.ranking_prediction_error_scale_percent);
            ...
            const double recent_consistency = std::clamp(
                std::exp(
                    -recent_disagreement / std::max(
                        1.0,
                        config.ranking_recent_disagreement_scale_percent)),
                0.0,
                1.0);
            const double recent_regret_excess = std::max(
                0.0,
                entry->last_regret_percent -
                    config.ranking_recent_regret_soft_limit_percent);
            ...
            double exact_weight = std::clamp(
                config.maximum_ranking_history_weight * authority,
                0.0,
                config.maximum_ranking_history_weight);
            if (entry->decayed_success_rate < 0.25 &&
                entry->decayed_regret_percent >
                    config.ranking_success_regret_percent)
            {
                exact_weight = std::max(
                    exact_weight,
                    std::min(
                        config.ranking_minimum_negative_evidence_weight,
                        config.maximum_ranking_history_weight));
            }
        ...
        static void apply_similarity_history(
            PlanCostEstimate& candidate,
            const WorkloadFingerprint& fingerprint,
            const Config& config)
        {
            ...
            candidate.similarity_rank_used =
                ...
                similar.confidence > 0.0;
            if (!candidate.similarity_rank_used)
                return;

            const double uncertainty = 1.0 -
                std::clamp(similar.confidence, 0.0, 1.0);
            candidate.similarity_rank_score = 1.0 +
                config.ranking_regret_penalty_scale *
                    similar.regret_percent / 100.0 +
                config.ranking_uncertainty_penalty_scale * uncertainty;
            candidate.similarity_rank_confidence = std::clamp(
                similar.confidence, 0.0, 1.0);
        }
        ...
        static EmpiricalEvidence empirical_evidence(
            const ExperienceEntry* entry,
            const Config& config)
        {
            EmpiricalEvidence result;
            if (entry == nullptr ||
                entry->sample_count < config.minimum_empirical_override_samples)
            {
                return result;
            }

            result.mean_ms = entry->outcome_sample_count > 0 &&
                    entry->decayed_elapsed_ms > 0.0
            ...
            const double standard_error = entry->standard_deviation_ms /
                std::sqrt(std::max(1.0, samples));
            const double half_width = std::max(
                0.0,
                config.empirical_override_z * standard_error);
                ...
                1.0 - std::exp(
                    -samples /
                        static_cast<double>(std::max<std::size_t>(
                            1,
                            config.minimum_empirical_override_samples))),
                0.0,
                1.0);
            const double coefficient_of_variation =
                entry->standard_deviation_ms / result.mean_ms;
            const double stability_confidence = std::clamp(
                1.0 / (1.0 + 2.0 * coefficient_of_variation),
                0.0,
                1.0);
            result.confidence = evidence_confidence * stability_confidence;
            result.available = result.confidence >=
                config.minimum_empirical_override_confidence;
            return result;
        }

        static void apply_empirical_plan_override(
            std::vector<PlanCostEstimate>& candidates,
            const WorkloadFingerprint& fingerprint,
            const Config& config)
        {
            ...
            for (std::size_t index = 0; index < candidates.size(); ++index)
            {
                if (candidates[index].available &&
                    positive(candidates[index].ranking_score) &&
            ...
            for (std::size_t index = 0; index < candidates.size(); ++index)
            {
                ...
                candidates[index].empirical_override_confidence =
                    evidence[index].confidence;
                candidates[index].empirical_runtime_ms =
                    evidence[index].mean_ms;
                candidates[index].empirical_runtime_ci_low_ms =
                    evidence[index].ci_low_ms;
                candidates[index].empirical_runtime_ci_high_ms =
                    evidence[index].ci_high_ms;
            ...
            if (empirical_best >= candidates.size() ||
                empirical_best == selected_index ||
                !evidence[selected_index].available)
            {
                return;
            }

            const double gain_percent = std::max(
                0.0,
                (evidence[selected_index].mean_ms -
                    evidence[empirical_best].mean_ms) /
                    evidence[empirical_best].mean_ms * 100.0);
            const bool intervals_separate =
                evidence[empirical_best].ci_high_ms <
                    evidence[selected_index].ci_low_ms;
            const bool strong_fallback =
                gain_percent >=
                    2.0 * config.minimum_empirical_override_gain_percent &&
                evidence[empirical_best].confidence >=
                    std::min(
                        1.0,
                        config.minimum_empirical_override_confidence + 0.10) &&
                evidence[selected_index].confidence >=
                    config.minimum_empirical_override_confidence;
            ...
            best.confidence = std::clamp(
                std::max(best.confidence, evidence[empirical_best].confidence),
                0.0,
                1.0);
        ...
        static void enforce_override_guard(
            std::vector<PlanCostEstimate>& candidates,
            ...
            const Config& config)
        {
            ...
            if (learned_index >= candidates.size() ||
            ...
            const double uncertainty_margin =
                (baseline.predicted_runtime_stddev_ms +
                 learned.predicted_runtime_stddev_ms) /
            ...
            learned.learned_override_required_margin_percent =
                ...
                config.override_uncertainty_margin_scale *
                    std::min(25.0, uncertainty_margin);
            learned.learned_override_allowed =
                learned.learned_override_confidence >=
                    config.minimum_learned_override_confidence &&
                learned.learned_override_gain_percent >=
                    learned.learned_override_required_margin_percent;
            ...
            if (!learned.learned_override_allowed)
            {
                ...
                baseline.learned_override_confidence =
                    learned.learned_override_confidence;
                baseline.learned_override_gain_percent =
                    learned.learned_override_gain_percent;
                baseline.learned_override_required_margin_percent =
                    learned.learned_override_required_margin_percent;
                baseline.ranking_score = std::min(
                    baseline.ranking_score,
                    learned.ranking_score * (1.0 - 1.0e-9));
            }
        }
