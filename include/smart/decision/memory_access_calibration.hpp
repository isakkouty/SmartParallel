...
{
  public:
    MemoryAccessCalibrationResult evaluate(const ExecutionPlan& plan,
                                           ...
                                           const PerformanceModel& model) const
    {
        ...
        const bool hinted_randomness =
            model.function.available && model.function.feature_confidence > 0.0;
        result.random_access_ratio =
            hinted_randomness   ? std::clamp(model.function.memory_randomness, 0.0, 1.0)
            : random_known == 0 ? 0.0
        ...
        const bool cache_resident = result.l3_pressure > 0.0 && result.l3_pressure <= 0.85
                                    && result.random_access_ratio < 0.45;
        const bool dependency_bound =
        ...
        const bool latency_bound =
            ((random_known > 0 || hinted_randomness) && result.random_access_ratio >= 0.50)
            || dependency_bound || storage_implies_irregular_access;
        const bool large_record = analysis.objects_are_large || result.bytes_per_iteration >= 128.0;
        ...
        if (random_known > 0 || hinted_randomness || storage_implies_irregular_access)
        {
        ...
        result.confidence = std::clamp(classification.confidence * structural_confidence, 0.0, 1.0);
        ...
        const std::size_t workers = plan.parallel ? std::max<std::size_t>(1, plan.job_count) : 1;
        result.worker_pressure =
            std::clamp(static_cast<double>(workers) / static_cast<double>(physical), 0.0, 2.0);
        ...
        switch (result.regime)
        {
            ...
            case MemoryAccessRegime::LatencyBound:
                ...
                if (plan.parallel && plan.strategy == ExecutionStrategy::StaticChunks)
                    raw_factor += 0.02;
                break;
        ...
        result.applied = result.confidence >= 0.15 && std::abs(result.factor - 1.0) > 1.0e-9;
        return result;
    }

    MemoryAccessCalibrationResult apply(PlanCostEstimate& estimate,
                                        const WorkloadFamilyClassification& classification,
                                        const WorkloadAnalysis& analysis,
                                        const PerformanceModel& model) const
    {
        const auto result = evaluate(estimate.plan, classification, analysis, model);
