...
namespace smart
{
    ...
    public:
            ...
            double prediction_confidence)
        {
            ...
            const PlanCostEstimate* best = find_candidate(
                candidates,
                recommended_plan);
            if (best == nullptr)
                return result;

            result.exploitation_score = best->ranking_score > 0.0
                ? best->ranking_score
                : best->predicted_total_ms;
            ...
            if (!config.enable_online_exploration ||
                candidates.size() < 2 ||
            ...
            const double max_ratio = 1.0 +
                std::max(0.0, config.maximum_exploration_score_gap_percent) /
                    100.0;
            ...
            for (const PlanCostEstimate& candidate : candidates)
            {
                ...
                const double score = candidate.ranking_score > 0.0
            ...
            const double probability = std::clamp(
                ...
                0.0,
                config.maximum_exploration_probability);
            ...
            result.selected_score = selected.ranking_score > 0.0
                ? selected.ranking_score
                : selected.predicted_total_ms;
            ...
            double exploitation_expected_ms)
        {
            ...
            if (!std::isfinite(actual_ms) || actual_ms < 0.0 ||
                !std::isfinite(exploitation_expected_ms) ||
            ...
            const double regret = std::max(
                0.0,
                (actual_ms - exploitation_expected_ms) /
                    exploitation_expected_ms * 100.0);
            ...
            if (regret > global_config().maximum_exploration_regret_percent)
            {
                ...
                state.cooldown_remaining =
                    global_config().exploration_cooldown_calls;
            }
            else
            {
    ...
    private:
        ...
        static bool same_plan(
            const ExecutionPlan& left,
            const ExecutionPlan& right)
        {
        ...
        static const PlanCostEstimate* find_candidate(
            const std::vector<PlanCostEstimate>& candidates,
        ...
        static bool deterministic_trial(
            ...
            double probability)
        {
            ...
            x ^= x >> 31;
            const double unit = static_cast<double>(x >> 11) /
