...
namespace smart
{
    ...
    {
    public:
        ResidualCorrectionResult evaluate(
            ...
            const WorkloadFingerprint& fingerprint) const
        {
            ...
            const ExperienceEntry* exact = database.find_plan(
                fingerprint,
                estimate.plan);

            const FamilyLimits family = family_limits(
                estimate.workload_family,
                estimate.workload_family_confidence,
                config);
                ...
                exact->prediction_sample_count >= minimum_exact_samples)
            {
                ...
                const double prediction_quality = std::clamp(
                    1.0 - exact->average_absolute_prediction_error_percent /
                        150.0,
                    0.15,
                    1.0);

                result.uncertainty = std::clamp(
                    1.0 - sample_strength * stability,
                    0.0,
                    1.0);
                result.confidence = std::clamp(
                    sample_strength * stability * prediction_quality,
                    0.0,
                    1.0);

                const double exact_factor = std::clamp(
                    exact->average_runtime_correction,
                    family.minimum_factor,
                    family.maximum_factor);
                 ...
                 result.confidence < config.minimum_residual_exact_confidence))
            {
                    ...
                    similar.similarity >= config.minimum_similarity)
                {
                    ...
                    const double similarity_weight = std::clamp(
                        similar.confidence * similar.similarity *
                    ...
                    result.confidence = std::clamp(
                        result.confidence + similarity_weight * 0.5,
                        0.0,
                        1.0);
                    result.uncertainty = std::clamp(
            ...
            const double maximum_weight = std::clamp(
            ...
            result.factor = std::exp(
                requested_log_factor * result.history_weight);
            result.factor = std::clamp(
                result.factor,
                family.minimum_factor,
                family.maximum_factor);
            result.applied = result.history_weight > 0.0 &&
                std::abs(result.factor - 1.0) > 1e-9;
            return result;
        }
        ...
        void apply(
            ...
            const WorkloadFingerprint& fingerprint) const
        {
            const ResidualCorrectionResult result = evaluate(
                estimate,
                fingerprint);
            ...
            estimate.residual_effective_minimum_factor =
                result.effective_minimum_factor;
            estimate.residual_effective_maximum_factor =
                result.effective_maximum_factor;
            ...
            estimate.confidence = std::clamp(
                std::max(
                    estimate.confidence,
                    result.confidence * (1.0 - result.uncertainty * 0.5)),
                0.0,
                1.0);
        }
    ...
    private:
        ...
        static FamilyLimits family_limits(
            ...
            const Config& config)
        {
            ...
            result.weight_scale = std::clamp(
                1.0 + (target_weight - 1.0) * confidence,
                0.0,
                1.0);
            result.minimum_factor = std::clamp(
                config.minimum_residual_correction_factor +
                    (target_minimum -
                     config.minimum_residual_correction_factor) * confidence,
                config.minimum_residual_correction_factor,
                1.0);
            result.maximum_factor = std::clamp(
                config.maximum_residual_correction_factor +
                    (target_maximum -
                     config.maximum_residual_correction_factor) * confidence,
                1.0,
                config.maximum_residual_correction_factor);
