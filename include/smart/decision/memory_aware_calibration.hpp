...
namespace smart
{
    ...
    {
    public:
        MemoryAwareCalibrationResult apply(
            ...
            const PerformanceModel& model) const
        {
            ...
            estimate.memory_feature_bytes_per_iteration =
                features.bytes_touched_per_iteration;
            estimate.memory_feature_arithmetic_intensity =
            ...
            const std::size_t logical = std::max<std::size_t>(
                1, model.hardware.logical_threads);
            const std::size_t physical = std::max<std::size_t>(
                1,
                model.hardware.physical_cores == 0
                    ? logical
                    : model.hardware.physical_cores);
            const std::size_t workers = estimate.plan.parallel
                ? std::max<std::size_t>(1, estimate.plan.job_count)
                : 1;
            result.worker_pressure = static_cast<double>(workers) /
            ...
            if (features.cache_resident &&
                features.cache_reuse_score >= 0.65 &&
                estimate.plan.parallel)
            {
                if (estimate.plan.strategy == ExecutionStrategy::StaticChunks)
                    raw_factor -= 0.015;
                if (result.worker_pressure > 1.0)
            ...
            estimate.predicted_speedup = estimate.predicted_execution_ms > 0.0
                ? std::max(1.0,
                    estimate.useful_work_ms / estimate.predicted_execution_ms)
