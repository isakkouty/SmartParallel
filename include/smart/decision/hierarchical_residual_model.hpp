...
namespace smart
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            Prediction predict(
                const std::array<double, N>& features,
                ...
                double feature_confidence) const
            {
                ...
                const double variance = residual_weight_ > 1.0
                    ? std::max(1.0e-4, residual_m2_ / residual_weight_)
                    : 0.12;
                ...
                if (solve_system(features, leverage_solution, ridge_lambda))
                {
                    leverage = std::max(
                        0.0,
                        dot(features, leverage_solution));
                }
                result.standard_deviation = std::sqrt(
                    variance * (1.0 + std::min(4.0, leverage)));
                ...
                const double sample_confidence = std::clamp(
                    effective_samples_ /
                        (effective_samples_ + static_cast<double>(
                            std::max<std::size_t>(1, full_confidence_samples))),
                    0.0,
                    1.0);
                const double stability_confidence = std::clamp(
                    std::exp(-variance / std::max(1.0e-6, variance_scale)),
                    0.0,
                    1.0);
            ...
            void update(
                ...
                double ridge_lambda)
            {
                if (!std::isfinite(target) || !std::isfinite(weight) ||
                    weight <= 0.0)
                {
                    return;
                }
                ...
                if (observations_ > 0 &&
                    solve_system(rhs_, coefficients, ridge_lambda))
                {
            ...
            {
                output << label << ' ' << observations_ << ' '
                       << effective_samples_ << ' ' << residual_mean_ << ' '
                       << residual_m2_ << ' ' << residual_weight_ << '\n';
            ...
            {
                std::string label;
                input >> label >> observations_ >> effective_samples_
    ...
    {
    public:
        HierarchicalResidualResult evaluate(
            const PlanCostEstimate& estimate) const
        {
        ...
        HierarchicalResidualResult apply(PlanCostEstimate& estimate) const
        {
            ...
            estimate.hierarchical_residual_log_stddev =
                result.log_standard_deviation;
            estimate.predicted_runtime_stddev_ms =
                result.runtime_standard_deviation_ms;
            estimate.hierarchical_shared_contribution =
                result.shared_contribution;
            estimate.hierarchical_backend_contribution =
                result.backend_contribution;
            estimate.hierarchical_family_backend_contribution =
                result.family_backend_contribution;
            estimate.hierarchical_shared_confidence =
                result.shared_confidence;
            estimate.hierarchical_backend_confidence =
                result.backend_confidence;
            estimate.hierarchical_family_backend_confidence =
                result.family_backend_confidence;
            estimate.hierarchical_shared_samples = result.shared_samples;
            estimate.hierarchical_backend_samples = result.backend_samples;
            estimate.hierarchical_family_backend_samples =
                result.family_backend_samples;
            ...
            estimate.predicted_total_ms =
                result.corrected_runtime_ms + estimate.framework_overhead_ms;
            return result;
        }

        void record(
            const PlanCostEstimate& estimate,
            double actual_runtime_ms)
        {
            if (!estimate.residual_features.available ||
                !std::isfinite(actual_runtime_ms) || actual_runtime_ms <= 0.0)
            {
                return;
            }

            const double base = estimate.hierarchical_residual_base_ms > 0.0
                ? estimate.hierarchical_residual_base_ms
                : std::max(
                    0.0,
                    estimate.analytical_baseline_total_ms -
                        estimate.framework_overhead_ms);
            if (!std::isfinite(base) || base <= 0.0)
                return;

            const double target = std::clamp(
                std::log(actual_runtime_ms / base),
                std::log(0.25),
                std::log(4.0));
            const Config& config = global_config();
            const std::size_t backend = backend_index(estimate.plan);
            const auto memberships =
                estimate.residual_features.family_membership.expert_weights();
            const auto shared_features = shared_feature_values(
                estimate.residual_features.values);
            const auto expert_features = expert_feature_values(
                estimate.residual_features.values);

            std::lock_guard<std::mutex> lock(mutex_);

            const auto shared_before = shared_.predict(
                shared_features,
                1,
            ...
            const double shared_prediction = shared_before.available
                ? shared_before.mean
                : 0.0;

            const auto backend_before = backends_[backend].predict(
                expert_features,
                1,
                ...
                config.hierarchical_residual_variance_scale,
                estimate.residual_features.feature_confidence);
            const double backend_prediction = backend_before.available
                ? backend_before.mean
                : 0.0;
            ...
            const double remaining = target - shared_prediction -
                backend_prediction;
            for (std::size_t family = 0; family < family_count; ++family)
            {
                const double membership = std::clamp(
                    memberships[family], 0.0, 1.0);
                if (membership < 0.05)
                    continue;
                family_backends_[family][backend].update(
                    expert_features,
                    remaining,
                    membership,
                    config.hierarchical_residual_decay,
        ...
        bool save_to_file(const std::string& path) const
        {
            ...
            {
                if (!backends_[backend].save(
                        output,
                        "backend_" + std::to_string(backend)))
                {
                    return false;
                }
            }
            for (std::size_t family = 0; family < family_count; ++family)
            {
                for (std::size_t backend = 0; backend < backend_count; ++backend)
                {
                    if (!family_backends_[family][backend].save(
                            output,
                            "family_" + std::to_string(family) +
                                "_backend_" + std::to_string(backend)))
                    {
                        return false;
                    }
                }
    ...
    private:
        ...
        HierarchicalResidualResult evaluate_unlocked(
            const PlanCostEstimate& estimate) const
        {
            HierarchicalResidualResult result;
            result.base_runtime_ms = std::max(
                0.0,
                estimate.predicted_total_ms - estimate.framework_overhead_ms);
            result.corrected_runtime_ms = result.base_runtime_ms;
            if (!estimate.residual_features.available ||
                result.base_runtime_ms <= 0.0)
            {
                return result;
            }
            ...
            const Config& config = global_config();
            const std::size_t backend = backend_index(estimate.plan);
            const double feature_confidence =
                estimate.residual_features.feature_confidence;
            const auto shared_features = shared_feature_values(
                estimate.residual_features.values);
            const auto expert_features = expert_feature_values(
            ...
            const auto shared = shared_.predict(
                shared_features,
                ...
                config.hierarchical_residual_variance_scale,
                feature_confidence);
            const auto backend_model = backends_[backend].predict(
                expert_features,
            ...
            result.backend_contribution =
                backend_model.mean * backend_model.confidence;
            ...
            for (std::size_t family = 0; family < family_count; ++family)
            {
                ...
                family_contribution += membership * expert.mean *
                    expert.confidence;
                family_confidence_mass += membership * expert.confidence;
                family_variance += membership * membership *
                    expert.standard_deviation * expert.standard_deviation;
                family_samples = std::max(family_samples, expert.samples);
            }
            result.family_backend_samples = family_samples;
            result.family_backend_confidence = std::clamp(
                family_confidence_mass, 0.0, 1.0);
            result.family_backend_contribution = family_contribution;

            const double combined_log_residual =
                result.shared_contribution +
                result.backend_contribution +
                result.family_backend_contribution;
            result.confidence = std::clamp(
                1.0 -
                    (1.0 - result.shared_confidence) *
                    (1.0 - result.backend_confidence) *
                    (1.0 - result.family_backend_confidence),
                0.0,
                1.0);

            const double weak_lower = std::log(std::max(
                0.01, config.hierarchical_weak_minimum_factor));
            const double weak_upper = std::log(std::max(
                1.0, config.hierarchical_weak_maximum_factor));
            const double mature_lower = std::log(std::max(
                0.01, config.hierarchical_mature_minimum_factor));
            const double mature_upper = std::log(std::max(
                1.0, config.hierarchical_mature_maximum_factor));
            const double lower = weak_lower +
                (mature_lower - weak_lower) * result.confidence;
            const double upper = weak_upper +
                (mature_upper - weak_upper) * result.confidence;
            result.log_residual = std::clamp(
                combined_log_residual, lower, upper);
            result.factor = std::exp(result.log_residual);
            result.corrected_runtime_ms =
                result.base_runtime_ms * result.factor;

            const double shared_variance =
                shared.standard_deviation * shared.standard_deviation *
                shared.confidence * shared.confidence;
            const double backend_variance =
                backend_model.standard_deviation *
                backend_model.standard_deviation *
                backend_model.confidence * backend_model.confidence;
            const double model_variance = shared_variance +
                backend_variance + family_variance;
            const double unsupported_variance =
                0.09 * (1.0 - result.confidence) *
                (1.0 - result.confidence);
            result.log_standard_deviation = std::sqrt(
                std::max(1.0e-4, model_variance + unsupported_variance));
            result.runtime_standard_deviation_ms =
                result.corrected_runtime_ms *
                std::sqrt(std::exp(
                    result.log_standard_deviation *
                    result.log_standard_deviation) - 1.0);
            result.applied = result.confidence > 0.0 &&
                std::abs(result.factor - 1.0) > 1.0e-6;
            return result;
        }

        static std::array<double, residual_feature_count>
        shared_feature_values(
            const std::array<double, residual_feature_count>& source)
        {
        ...
        std::array<std::array<Model, backend_count>, family_count>
            family_backends_{};
    };
