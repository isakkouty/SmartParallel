...
namespace smart::validation
{
    ...
    {
        if (regrets.empty()) throw std::invalid_argument("regret sample cannot be empty");
        ...
        result.catastrophic_rate = static_cast<double>(std::count_if(regrets.begin(), regrets.end(),
            [catastrophic_threshold](double value) { return value > catastrophic_threshold; })) / regrets.size();
        return result;
    }
}
