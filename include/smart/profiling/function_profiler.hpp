...
namespace smart
{
    ...
    struct FunctionProfile
    {
        ...
        FunctionProfileSamplingMode sampling_mode =
            FunctionProfileSamplingMode::None;

        FunctionProfileUnavailableReason unavailable_reason =
            FunctionProfileUnavailableReason::None;
    ...
    public:
            ...
            Config config = {}) const
        {
            ...
            std::size_t batch_size =
                std::min(config.batch_size, total);
            ...
            for (;;)
            {
                const std::size_t pilot_batch =
                    std::min(batch_size, total);

                const BatchMeasurement pilot = measure_batch(
                    begin,
                    pilot_batch,
                    func,
                    calibration.timer_overhead_ms);
                ...
                if (first_pilot_batch == 0)
                {
                    ...
                    first_pilot_cost = pilot.net_ms /
                        static_cast<double>(pilot_batch);
                }
            ...
            profile.signal_to_floor_ratio =
                calibration.measurement_floor_ms > 0.0
                    ? final_pilot_raw_ms /
                        calibration.measurement_floor_ms
                    : std::numeric_limits<double>::infinity();

            profile.measurement_reliable =
                final_pilot_net_ms > 0.0 &&
                profile.signal_to_floor_ratio >=
                    config.minimum_signal_to_floor_ratio;
            ...
            if (final_pilot_batch > 0)
            {
                records.push_back({
                    final_pilot_net_ms /
                        static_cast<double>(final_pilot_batch),
                    begin,
                    SampleKind::Pilot,
            ...
            {
                if (profile.callback_invocations +
                        profile.chosen_batch_size >
                    config.max_callback_invocations)
                {
                    profile.stop_reason =
                        ProfileStopReason::InvocationBudgetReached;
                    break;
                }
                ...
                {
                    profile.stop_reason =
                        ProfileStopReason::TimeBudgetReached;
                    break;
                }
                ...
                const std::size_t batch_count =
                    std::min(profile.chosen_batch_size, end - start_index);
                ...
                const BatchMeasurement measurement = measure_batch(
                    start_index,
                    batch_count,
                    func,
                    calibration.timer_overhead_ms);
                ...
                if (records.size() >= config.min_samples &&
                    relative_standard_error(values_from(records)) <=
                        config.relative_error_target)
                {
                    profile.stop_reason =
                        ProfileStopReason::ConfidenceReached;
                    break;
                }
            }
            ...
            {
                profile.stop_reason =
                    records.size() >= config.max_samples
    ...
    private:
        ...
        static void normalize_config(Config& config)
        {
            ...
            config.max_batch_size =
                std::max(config.batch_size, config.max_batch_size);
            config.max_callback_invocations =
                std::max(config.batch_size, config.max_callback_invocations);
            config.local_sample_count = std::min(
                config.local_sample_count,
                config.max_samples > 1 ? config.max_samples - 1 : 0);
            config.max_profile_time_ms =
                std::max(0.01, config.max_profile_time_ms);
            config.target_batch_duration_ms =
                std::max(0.001, config.target_batch_duration_ms);
            config.relative_error_target =
                std::max(0.001, config.relative_error_target);
            config.trim_fraction =
                std::clamp(config.trim_fraction, 0.0, 0.45);
            config.warmup_ratio_threshold =
                std::max(1.0, config.warmup_ratio_threshold);
        }
        ...
        static ProfilerCalibration profiler_calibration()
        {
            static const ProfilerCalibration calibration = []
            {
                ...
                result.measurement_floor_ms =
                    std::isfinite(minimum_positive)
                        ? std::max(minimum_positive,
                            result.timer_overhead_ms)
        ...
        template <typename Function>
        static BatchMeasurement measure_batch(
        ...
        static double elapsed_since_ms(
        ...
        static double relative_standard_error(
            const std::vector<double>& values)
        {
            ...
            return deviation /
                (average * std::sqrt(static_cast<double>(values.size())));
        }
            ...
            double trim_fraction)
        {
            ...
            const auto first = sorted.begin() +
                static_cast<std::ptrdiff_t>(trim_count);
            const auto last = sorted.end() -
                static_cast<std::ptrdiff_t>(trim_count);
        ...
        static ObservationConfidence determine_confidence(
            ...
            double relative_error)
        {
            ...
            if (profile.samples >= config.min_samples &&
                relative_error <= config.relative_error_target &&
            ...
            const Config& config)
        {
            ...
            profile.trimmed_mean_ms_per_iteration =
                trimmed_mean(samples, config.trim_fraction);
            profile.stddev_ms_per_iteration =
                standard_deviation(samples, profile.avg_ms_per_iteration);
            ...
            if (profile.median_ms_per_iteration > 0.0)
            {
                profile.tail_ratio =
                    ...
                    profile.median_ms_per_iteration;
            }
            ...
            const double primary_cost =
                profile.trimmed_mean_ms_per_iteration > 0.0
                    ? profile.trimmed_mean_ms_per_iteration
                    : profile.avg_ms_per_iteration;

            profile.estimated_total_work_ms =
                primary_cost * static_cast<double>(total);
            ...
            profile.stable =
                profile.coefficient_of_variation <= config.stable_cv &&
            ...
            profile.metadata.confidence = determine_confidence(
                profile,
                config,
                relative_error);
        }

        static void finalize_warmup_observations(
            FunctionProfile& profile,
            const std::vector<SampleRecord>& records,
            const Config& config)
        {
            ...
            profile.steady_state_ms_per_iteration =
                percentile(steady_samples, 0.50);

            if (profile.steady_state_ms_per_iteration > 0.0)
            {
                profile.warmup_ratio =
                    profile.first_batch_ms_per_iteration /
                    profile.steady_state_ms_per_iteration;
            }

            profile.warmup_detected =
                profile.measurement_reliable &&
                steady_samples.size() >= 3 &&
                profile.warmup_ratio >= config.warmup_ratio_threshold;

            if (profile.first_batch_ms_per_iteration >
                profile.steady_state_ms_per_iteration)
            {
        ...
        static void finalize_spatial_observations(
            FunctionProfile& profile,
            const std::vector<SampleRecord>& records)
        {
            ...
            for (std::size_t i = 0; i < regions.size(); ++i)
            {
                ...
                if (medians[i] > 0.0)
                {
                    minimum_positive = std::min(
                        minimum_positive, medians[i]);
            ...
            if (available_regions >= 2 &&
                std::isfinite(minimum_positive) &&
                minimum_positive > 0.0)
            {
                profile.regional_cost_ratio =
                    maximum_value / minimum_positive;
            }

            profile.spatial_observations_available =
                !local.empty() || !distributed.empty() ||
                available_regions >= 2;
        }

        static std::vector<double> values_from(
            const std::vector<SampleRecord>& records)
        {
        ...
        static SampleRegion region_for_index(
            std::size_t index,
            std::size_t total)
        {
            if (total <= 1)
        ...
        static std::size_t local_sample_index(
            std::size_t sample,
            ...
            std::size_t batch_count)
        {
            ...
            const std::size_t group_width =
                sample_count > 0 ? sample_count * batch_count : batch_count;
            const std::size_t group_begin =
                anchor > group_width / 2
                    ? anchor - group_width / 2
                    : 0;
        ...
        static std::size_t jittered_stratified_index(
            std::size_t sample,
            std::size_t sample_count,
            std::size_t total,
            std::size_t batch_count)
        {
            ...
            const std::size_t maximum_start = total - batch_count;
            const std::size_t stratum_begin =
                sample * (maximum_start + 1) / sample_count;
            const std::size_t stratum_end =
                ((sample + 1) * (maximum_start + 1) / sample_count) - 1;
            ...
            const std::size_t jitter = static_cast<std::size_t>(
                deterministic_hash(sample + total) % span);
