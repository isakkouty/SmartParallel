...
class PerformanceModelBuilder
{
  public:
    ...
    PerformanceModel build(const WorkloadAnalysis& analysis, const ExecutionHints& hints) const
    {
        ...
        model.workload.iterations = analysis.structural.logical_iterations;
        const std::size_t represented = analysis.structural.represented_input_bytes;
        const std::size_t external = hints.available ? hints.external_working_set_bytes : 0;
        model.workload.working_set_bytes = external > static_cast<std::size_t>(-1) - represented
        ...
        model.hardware = hw;
        model.structural_observation_metadata = analysis.structural.metadata;
        ...
        model.function.external_working_set_bytes = hints.external_working_set_bytes;
        model.function.bytes_touched_per_iteration = hints.bytes_touched_per_iteration;
        ...
        {
            model.l1_pressure = analysis.structural.l1_residency_ratio;
            model.l2_pressure = analysis.structural.l2_residency_ratio;
            model.l3_pressure = analysis.structural.l3_residency_ratio;
            ...
            if (external > 0)
            {
                model.l1_pressure = std::max(model.l1_pressure,
                                             static_cast<double>(model.workload.working_set_bytes)
                                                 / static_cast<double>(l1_cache_size));
                model.l2_pressure = std::max(model.l2_pressure,
                                             static_cast<double>(model.workload.working_set_bytes)
                                                 / static_cast<double>(l2_cache_size));
                model.l3_pressure = std::max(model.l3_pressure,
                                             static_cast<double>(model.workload.working_set_bytes)
        ...
        {
            model.l1_pressure = static_cast<double>(model.workload.working_set_bytes)
                                / static_cast<double>(l1_cache_size);

            model.l2_pressure = static_cast<double>(model.workload.working_set_bytes)
                                / static_cast<double>(l2_cache_size);

            model.l3_pressure = static_cast<double>(model.workload.working_set_bytes)
                                / static_cast<double>(l3_cache_size);
        }

        model.page_pressure =
            static_cast<double>(model.workload.working_set_bytes) / static_cast<double>(page_size);

        model.cache_pressure = model.l3_pressure >= 1.0 ? 1.0 : model.l3_pressure;

        model.working_set_exceeds_l3 = model.l3_pressure >= 1.0;

        model.likely_memory_sensitive =
            model.working_set_exceeds_l3 || model.workload.has_large_objects;
