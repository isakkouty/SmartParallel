...
namespace smart
{
    ...
    inline SimilarityComponent logarithmic_similarity(
        double left_log2,
        bool left_available,
        double right_log2,
        ...
        double distance_scale)
    {
        ...
        result.similarity = std::clamp(
            std::exp(-log_distance / safe_scale),
            0.0,
            1.0);
    ...
    inline SimilarityComponent size_bucket_similarity(
        std::size_t left,
        std::size_t right,
        double distance_scale = 2.0)
    {
        return logarithmic_similarity(
            left > 0 ? std::log2(static_cast<double>(left)) : 0.0,
            left > 0,
    ...
    inline bool decode_real_bucket_log2(
        std::size_t bucket,
        double& decoded_log2)
    {
        if (bucket == 0)
        ...
        const double mantissa =
            (static_cast<double>(mantissa_bucket) + 0.5) / 16.0;
        if (!(mantissa > 0.0))
            return false;
    ...
    inline SimilarityComponent real_bucket_similarity(
        std::size_t left,
        std::size_t right,
        double distance_scale = 1.5)
    {
    ...
    inline double fingerprint_bucket_similarity(
        std::size_t left,
        std::size_t right)
    {
    ...
    inline FingerprintSimilarity compare_fingerprints(
        const WorkloadFingerprint& left,
        const WorkloadFingerprint& right)
    {
        ...
        const SimilarityComponent iteration = size_bucket_similarity(
            left.iteration_bucket,
            right.iteration_bucket,
            2.0);
        const SimilarityComponent working_set = size_bucket_similarity(
            left.working_set_bucket,
            right.working_set_bucket,
            2.0);
        const SimilarityComponent object_size = size_bucket_similarity(
            left.object_size_bucket,
            right.object_size_bucket,
            1.5);
        const SimilarityComponent function_cost = real_bucket_similarity(
            left.function_cost_bucket,
            right.function_cost_bucket,
            1.5);
        const SimilarityComponent variation = real_bucket_similarity(
            left.variation_bucket,
            right.variation_bucket,
            2.0);
        const SimilarityComponent cache_pressure = real_bucket_similarity(
            left.cache_pressure_bucket,
            right.cache_pressure_bucket,
            1.5);
        const SimilarityComponent stride = size_bucket_similarity(
            left.stride_bucket,
            right.stride_bucket,
            1.5);
        ...
        const double weighted_coverage =
            iteration_weight * iteration.coverage +
            working_set_weight * working_set.coverage +
            object_size_weight * object_size.coverage +
            function_cost_weight * function_cost.coverage +
            variation_weight * variation.coverage +
            access_pattern_weight * access_pattern.coverage +
            stride_weight * stride.coverage +
            cache_pressure_weight * cache_pressure.coverage +
            topology_weight * topology.coverage +
            profile_shape_weight * profile_shape.coverage;
        ...
        const double normalized_similarity =
            weighted_similarity / weighted_coverage;
        const double coverage_penalty = 0.50 + 0.50 * result.evidence_coverage;
        result.total = std::clamp(
            normalized_similarity * coverage_penalty,
            0.0,
            1.0);
        result.normalized_distance = std::clamp(
            weighted_distance / weighted_coverage,
            0.0,
            1.0);
        return result;
    }

    inline double fingerprint_similarity(
        const WorkloadFingerprint& left,
        const WorkloadFingerprint& right)
    {
