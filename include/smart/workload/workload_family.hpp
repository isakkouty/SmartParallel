...
namespace smart
{
    ...
    public:
            ...
            const ExecutionHints* hints = nullptr) const
        {
            ...
            if (hints_available)
            {
                ...
                const double vectorization = clamp01(
                    hints->vectorization_potential);
                const double dependency = normalized_dependency(*hints);
                const double hint_confidence = clamp01(
                    hints->feature_confidence);

                score.compute += hint_confidence *
                    (2.4 * arithmetic + 0.5 * vectorization);
                score.branch += hint_confidence * 2.5 * branchiness;
                score.irregular_memory += hint_confidence *
                    (2.6 * randomness + 1.2 * dependency);
                score.streaming_memory += hint_confidence *
            ...
            const double contiguous_ratio = ratio(
                contiguous, known_contiguous);
            const double random_ratio = ratio(random, known_random);
            const double regular_stride_ratio = ratio(
                regular_stride, known_stride);
            ...
            if (profile != nullptr && profile->available)
            {
                ...
                const double cv = std::max(
                    0.0, profile->coefficient_of_variation);
                const double tail = std::max(1.0, profile->tail_ratio);
                const double regional = std::max(
                    1.0, profile->regional_cost_ratio);
                ...
                else
                {
                    ...
                    score.irregular_memory += std::min(
                        0.8, (tail - 1.0) * 0.5);
                    score.mixed += std::min(
                        0.8, (regional - 1.0) * 0.4);
                }
            }
            ...
            const double total = score.compute + score.streaming_memory +
            ...
            const double probability_margin = membership_margin(
                result.membership);
            result.confidence = clamp01(
                0.35 * dominance +
                0.35 * margin_ratio +
                0.30 * probability_margin);
            result.ambiguous = margin_ratio < 0.18 ||
    ...
    private:
        ...
        static WorkloadFamilyMembership soft_membership(
            const std::array<double, 5>& evidence)
        {
            WorkloadFamilyMembership membership;
            ...
            for (std::size_t index = 0; index < evidence.size(); ++index)
            {
                weights[index] = std::exp(
                    (evidence[index] - maximum) / temperature);
                sum += weights[index];
            }
        ...
        static double membership_margin(
            const WorkloadFamilyMembership& membership)
        {
