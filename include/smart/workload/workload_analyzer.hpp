...
namespace smart
{
    ...
    public:
        WorkloadAnalysis analyze(const Workload& workload) const
        {
            ...
            analysis.iteration_count_saturated =
                workload.iterations_saturated;
            ...
            analysis.structural.iterations_per_logical_thread =
                static_cast<double>(workload.iterations) /
                static_cast<double>(logical_threads);

            analysis.structural.iterations_per_physical_core =
                static_cast<double>(workload.iterations) /
                static_cast<double>(physical_cores);

            std::size_t large_object_threshold =
                hardware.cache_line_size * 2;
            ...
            for (std::size_t index = 0;
                 index < workload.dimensions.size();
                 ++index)
            {
            ...
            analysis.working_set_bytes =
                analysis.structural.represented_input_bytes;
            analysis.working_set_saturated =
                analysis.structural.represented_input_bytes_saturated;

            if (hardware.l1_cache_size > 0 &&
                hardware.l2_cache_size > 0 &&
