...
set "BUILD_DIR=build_v1_opencv_test3"
...
echo.
echo ==== [1/3] Configure OpenCV Test 3 ====
...
cmake -S . -B "%BUILD_DIR%" ^
  ...
  -DSMARTPARALLEL_BUILD_OPENCV_TEST3=ON
if errorlevel 1 exit /b 1

echo.
echo ==== [2/3] Build OpenCV Test 3 ====
cmake --build "%BUILD_DIR%" --target smartparallel_opencv_test3
if errorlevel 1 exit /b 1

echo.
echo ==== [3/3] Run OpenCV Test 3 ====
if not exist validation\output mkdir validation\output
"%BUILD_DIR%\smartparallel_opencv_test3.exe" "validation\output\opencv_test3_sobel.csv"
if errorlevel 1 exit /b 1

echo.
echo ============================================================
echo OpenCV Test 3 completed.
echo Results: validation\output\opencv_test3_sobel.csv
echo ============================================================
