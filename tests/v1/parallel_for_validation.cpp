...
void exactly_once(std::size_t begin, std::size_t end)
{
    ...
    smart::parallel_for(begin,
                        ...
                        [&](std::size_t i)
                        {
                            visits[i - begin].fetch_add(1);
                        });
    for (auto& v : visits)
        require(v.load() == 1, "index skipped or duplicated");
}
...
int main()
{
    try
    {
        ...
        exactly_once(0, 1);
        exactly_once(17, 18);
        exactly_once(11, 4099);
        bool threw = false;
        try
        {
            smart::parallel_for(9, 2, [](std::size_t) {});
        }
        ...
        require(!smart::global_last_decision_report().plan.parallel,
                "cheap loop should remain sequential");
        ...
        require(smart::global_last_parallel_for_profile_diagnostics().profile_available,
                "heavy callback was not profiled");
        require(smart::global_last_decision_report().plan.parallel,
                "heavy loop should become parallel");
        ...
        require(smart::global_last_parallel_for_profile_diagnostics().estimated_sequential_ms
                    >= 0.0,
