...
namespace
{
    ...
    void sobel_linear_range(const cv::Mat& padded, cv::Mat& destination,
                            std::size_t begin, std::size_t end)
    {
        ...
        for (std::size_t index = begin; index < end; ++index)
        {
            ...
            const float gx = -top[0] + top[2]
                - 2.0F * middle[0] + 2.0F * middle[2]
                - bottom[0] + bottom[2];
            const float gy = -top[0] - 2.0F * top[1] - top[2]
                + bottom[0] + 2.0F * bottom[1] + bottom[2];
    ...
    void sobel_opencv_parallel(const cv::Mat& padded, cv::Mat& destination)
    {
        ...
        {
            sobel_linear_range(padded, destination,
...
int main(int argc, char** argv)
{
    try
    {
        ...
        for (const Case& test_case : cases)
        {
            ...
            const double opencv_sobel_ms = median_runtime_ms(test_case.repetitions, [&]
            {
            ...
            const bool correct = manual_difference <= 1.0e-4 &&
                opencv_parallel_difference <= 1.0e-4 && opencv_difference <= 2.0e-3;
            all_correct = all_correct && correct;

            results.push_back(Result{test_case.name, test_case.width, test_case.height, source.total(),
                                     sequential_ms, opencv_parallel_ms, smartparallel_ms, opencv_sobel_ms,
