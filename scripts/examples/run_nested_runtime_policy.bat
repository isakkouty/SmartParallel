...
rem SmartParallel v1.1 Step 3 - configure, build, and run the nested runtime policy example.
...
set "BUILD_DIR=%REPO_ROOT%\build\nested_runtime_policy"

echo(
echo ==== [1/3] Configure nested runtime policy example ====
...
echo(
echo ==== [2/3] Build nested runtime policy example ====
cmake --build "%BUILD_DIR%" --target smartparallel_example
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run nested runtime policy example ====
"%BUILD_DIR%\examples\smartparallel_example.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Nested runtime policy example completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Nested runtime policy example failed. Review the first error above.
exit /b 1
