...
set "BUILD_DIR=%REPO_ROOT%\build\scheduling_regression_gate"

echo(
echo ==== [1/3] Configure scheduling benchmark/regression gate ====
...
echo(
echo ==== [2/3] Build scheduling benchmark/regression gate ====
cmake --build "%BUILD_DIR%" --target smartparallel_scheduling_regression_gate
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run scheduling benchmark/regression gate ====
"%BUILD_DIR%\examples\smartparallel_scheduling_regression_gate.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Scheduling benchmark/regression gate completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Scheduling benchmark/regression gate failed. Review the first error above.
exit /b 1
