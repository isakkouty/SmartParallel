...
set "BUILD_DIR=%REPO_ROOT%\build\backend_neutral_nested_coordinator"

echo(
echo ==== [1/3] Configure backend-neutral nested coordinator ====
...
echo(
echo ==== [2/3] Build backend-neutral nested coordinator ====
cmake --build "%BUILD_DIR%" --target smartparallel_backend_neutral_nested_coordinator
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run backend-neutral nested coordinator ====
"%BUILD_DIR%\examples\smartparallel_backend_neutral_nested_coordinator.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Backend-neutral nested coordinator completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Backend-neutral nested coordinator failed. Review the first error above.
exit /b 1
