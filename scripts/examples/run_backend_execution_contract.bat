...
set "BUILD_DIR=%REPO_ROOT%\build\backend_execution_contract"

echo(
echo ==== [1/3] Configure backend-neutral execution contract ====
...
echo(
echo ==== [2/3] Build backend-neutral execution contract ====
cmake --build "%BUILD_DIR%" --target smartparallel_backend_execution_contract
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run backend-neutral execution contract ====
"%BUILD_DIR%\examples\smartparallel_backend_execution_contract.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Backend-neutral execution contract completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Backend-neutral execution contract failed. Review the first error above.
exit /b 1
