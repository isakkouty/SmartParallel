...
rem SmartParallel v1.1 revised Step 9 - configure, build, and run shared ThreadPool work queues.
...
set "BUILD_DIR=%REPO_ROOT%\build\shared_thread_pool_work_queues"

echo(
echo ==== [1/3] Configure shared ThreadPool work queues example ====
...
echo(
echo ==== [2/3] Build shared ThreadPool work queues example ====
cmake --build "%BUILD_DIR%" --target smartparallel_example
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run shared ThreadPool work queues example ====
"%BUILD_DIR%\examples\smartparallel_example.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Shared ThreadPool work queues example completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Shared ThreadPool work queues example failed. Review the first error above.
exit /b 1
