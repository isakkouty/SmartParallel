...
set "BUILD_DIR=%REPO_ROOT%\build\performance_tuning_scheduler_validation"

echo(
echo ==== [1/3] Configure performance tuning and scheduler validation ====
...
echo(
echo ==== [2/3] Build performance tuning and scheduler validation ====
cmake --build "%BUILD_DIR%" --target smartparallel_performance_tuning_scheduler_validation
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run performance tuning and scheduler validation ====
"%BUILD_DIR%\examples\smartparallel_performance_tuning_scheduler_validation.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Performance tuning and scheduler validation completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Performance tuning and scheduler validation failed. Review the first error above.
exit /b 1
