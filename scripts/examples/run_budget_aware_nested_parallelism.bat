...
rem SmartParallel v1.1 Step 6 - configure, build, and run the budget-aware nested parallelism example.
...
set "BUILD_DIR=%REPO_ROOT%\build\budget_aware_nested_parallelism"

echo(
echo ==== [1/3] Configure budget-aware nested parallelism example ====
...
echo(
echo ==== [2/3] Build budget-aware nested parallelism example ====
cmake --build "%BUILD_DIR%" --target smartparallel_example
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run budget-aware nested parallelism example ====
"%BUILD_DIR%\examples\smartparallel_example.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Budget-aware nested parallelism example completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Budget-aware nested parallelism example failed. Review the first error above.
exit /b 1
