...
rem SmartParallel v1.1 revised Step 8 - configure, build, and run scheduler-visible work chunks.
...
set "BUILD_DIR=%REPO_ROOT%\build\scheduler_visible_work_chunks"

echo(
echo ==== [1/3] Configure scheduler-visible work chunks example ====
...
echo(
echo ==== [2/3] Build scheduler-visible work chunks example ====
cmake --build "%BUILD_DIR%" --target smartparallel_example
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run scheduler-visible work chunks example ====
"%BUILD_DIR%\examples\smartparallel_example.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Scheduler-visible work chunks example completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Scheduler-visible work chunks example failed. Review the first error above.
exit /b 1
