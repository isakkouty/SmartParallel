...
set "BUILD_DIR=%REPO_ROOT%\build\thread_pool_backend_integration"

echo(
echo ==== [1/3] Configure ThreadPool backend integration example ====
...
echo(
echo ==== [2/3] Build ThreadPool backend integration example ====
cmake --build "%BUILD_DIR%" --target smartparallel_thread_pool_backend_integration
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run ThreadPool backend integration ====
"%BUILD_DIR%\examples\smartparallel_thread_pool_backend_integration.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo ThreadPool backend integration completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo ThreadPool backend integration failed. Review the first error above.
exit /b 1
