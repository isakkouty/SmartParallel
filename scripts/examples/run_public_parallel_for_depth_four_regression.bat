...
set "BUILD_DIR=%REPO_ROOT%\build\public_parallel_for_depth_four_regression"

echo(
echo ==== [1/3] Configure automatic depth-four public path regression ====
...
echo(
echo ==== [2/3] Build automatic depth-four public path regression ====
cmake --build "%BUILD_DIR%" --target smartparallel_public_parallel_for_depth_four_regression
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run automatic depth-four public path regression ====
"%BUILD_DIR%\examples\smartparallel_public_parallel_for_depth_four_regression.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Automatic depth-four public path regression completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Automatic depth-four public path regression failed. Review the first error above.
exit /b 1
