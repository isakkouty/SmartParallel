...
rem SmartParallel v1.1 revised Step 10 - configure, build, and run nested ThreadPool helping.
...
set "BUILD_DIR=%REPO_ROOT%\build\nested_thread_pool_helping"

echo(
echo ==== [1/3] Configure nested ThreadPool helping example ====
...
echo(
echo ==== [2/3] Build nested ThreadPool helping example ====
cmake --build "%BUILD_DIR%" --target smartparallel_example
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run nested ThreadPool helping example ====
"%BUILD_DIR%\examples\smartparallel_example.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Nested ThreadPool helping example completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Nested ThreadPool helping example failed. Review the first error above.
exit /b 1
