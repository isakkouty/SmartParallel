...
rem SmartParallel v1.1 Step 7 - configure, build, and run the nested budget partitioning example.
...
set "BUILD_DIR=%REPO_ROOT%\build\nested_budget_partitioning"

echo(
echo ==== [1/3] Configure nested budget partitioning example ====
...
echo(
echo ==== [2/3] Build nested budget partitioning example ====
cmake --build "%BUILD_DIR%" --target smartparallel_example
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run nested budget partitioning example ====
"%BUILD_DIR%\examples\smartparallel_example.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Nested budget partitioning example completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Nested budget partitioning example failed. Review the first error above.
exit /b 1
