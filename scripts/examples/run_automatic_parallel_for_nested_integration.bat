...
set "BUILD_DIR=%REPO_ROOT%\build\automatic_parallel_for_nested_integration"

echo(
echo ==== [1/3] Configure automatic parallel_for nested integration ====
...
echo(
echo ==== [2/3] Build automatic parallel_for nested integration ====
cmake --build "%BUILD_DIR%" --target smartparallel_automatic_parallel_for_nested_integration
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run automatic parallel_for nested integration ====
"%BUILD_DIR%\examples\smartparallel_automatic_parallel_for_nested_integration.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo automatic parallel_for nested integration completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo automatic parallel_for nested integration failed. Review the first error above.
exit /b 1
