...
set "BUILD_DIR=%REPO_ROOT%\build\backend_negotiation_capability_resolution"

echo(
echo ==== [1/3] Configure backend negotiation example ====
...
echo(
echo ==== [2/3] Build backend negotiation example ====
cmake --build "%BUILD_DIR%" --target smartparallel_backend_negotiation_capability_resolution
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run backend negotiation and capability resolution ====
"%BUILD_DIR%\examples\smartparallel_backend_negotiation_capability_resolution.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Backend negotiation and capability resolution completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Backend negotiation and capability resolution failed. Review the first error above.
exit /b 1
