...
set "BUILD_DIR=%REPO_ROOT%\build\dependency_local_helping_continuation_priority"

echo(
echo ==== [1/3] Configure dependency-local helping and continuation priority ====
...
echo(
echo ==== [2/3] Build dependency-local helping and continuation priority ====
cmake --build "%BUILD_DIR%" --target smartparallel_dependency_local_helping_continuation_priority
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run dependency-local helping and continuation priority ====
"%BUILD_DIR%\examples\smartparallel_dependency_local_helping_continuation_priority.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Dependency-local helping and continuation priority completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Dependency-local helping and continuation priority failed. Review the first error above.
exit /b 1
