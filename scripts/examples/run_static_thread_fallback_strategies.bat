...
set "BUILD_DIR=%REPO_ROOT%\build\static_thread_fallback_strategies"

echo(
echo ==== [1/3] Configure StaticThread and fallback strategies ====
...
echo(
echo ==== [2/3] Build StaticThread and fallback strategies ====
cmake --build "%BUILD_DIR%" --target smartparallel_static_thread_fallback_strategies
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run StaticThread and fallback strategies ====
"%BUILD_DIR%\examples\smartparallel_static_thread_fallback_strategies.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo StaticThread and fallback strategies completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo StaticThread and fallback strategies failed. Review the first error above.
exit /b 1
