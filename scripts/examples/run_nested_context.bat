...
rem SmartParallel v1.1 Step 1 - configure, build, and run the nested context example.
...
echo(
echo ==== [1/3] Configure nested context example ====
...
echo(
echo ==== [2/3] Build nested context example ====
cmake --build "%BUILD_DIR%" --target smartparallel_example
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run nested context example ====
"%BUILD_DIR%\examples\smartparallel_example.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Nested context example completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Nested context example failed. Review the first error above.
exit /b 1
