...
rem SmartParallel v1.1 Step 4 - configure, build, and run the nested execution coordinator example.
...
set "BUILD_DIR=%REPO_ROOT%\build\nested_execution_coordinator"

echo(
echo ==== [1/3] Configure nested execution coordinator example ====
...
echo(
echo ==== [2/3] Build nested execution coordinator example ====
cmake --build "%BUILD_DIR%" --target smartparallel_example
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run nested execution coordinator example ====
"%BUILD_DIR%\examples\smartparallel_example.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Nested execution coordinator example completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Nested execution coordinator example failed. Review the first error above.
exit /b 1
