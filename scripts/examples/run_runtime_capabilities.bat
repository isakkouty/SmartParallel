...
rem SmartParallel v1.1 Step 2 - configure, build, and run the runtime capability example.
...
set "BUILD_DIR=%REPO_ROOT%\build\runtime_capabilities"

echo(
echo ==== [1/3] Configure runtime capability example ====
...
echo(
echo ==== [2/3] Build runtime capability example ====
cmake --build "%BUILD_DIR%" --target smartparallel_example
if errorlevel 1 goto :fail

echo(
echo ==== [3/3] Run runtime capability example ====
"%BUILD_DIR%\examples\smartparallel_example.exe"
if errorlevel 1 goto :fail

echo(
echo ============================================================
echo Runtime capability example completed successfully.
echo ============================================================
exit /b 0
...
:fail
echo(
echo Runtime capability example failed. Review the first error above.
exit /b 1
