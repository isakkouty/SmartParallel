...
namespace
{
    ...
    PhaseSample capture_last()
    {
        ...
        result.scheduler_ms = std::max(
            0.0, result.total_ms - result.profiling_ms - result.execution_ms);
    ...
    PhaseSample median_sample(const std::vector<PhaseSample>& samples)
    {
        ...
        result.cache_hit = std::count_if(samples.begin(), samples.end(),
            [](const auto& sample) { return sample.cache_hit; }) > samples.size() / 2;
        result.sequential_fast_path = std::count_if(samples.begin(), samples.end(),
            [](const auto& sample) { return sample.sequential_fast_path; }) > samples.size() / 2;
        return result;
    }
}
...
int main(int argc, char** argv)
{
    ...
    for (std::size_t n : {10u, 100u, 1000u, 10000u, 100000u})
    {
        ...
        const std::size_t confirmation_calls = std::max<std::size_t>(
            1, config.parallel_for_sequential_fast_path_min_observations);
        ...
        out << n << ',' << std::setprecision(10)
            << sequential << ','
            << cold.total_ms << ',' << cold.scheduler_ms << ','
            << cold.cache_lookup_ms << ',' << cold.analysis_ms << ','
            << cold.profiling_ms << ',' << cold.decision_ms << ','
            << cold.execution_ms << ','
            << cached.total_ms << ',' << cached.scheduler_ms << ','
            << cached.cache_lookup_ms << ',' << cached.analysis_ms << ','
            << cached.profiling_ms << ',' << cached.decision_ms << ','
            << cached.execution_ms << ','
        ...
        std::cout << n
            << " seq=" << sequential << " ms"
            << " cold(total/scheduler/profile/exec)="
            << cold.total_ms << '/' << cold.scheduler_ms << '/'
            << cold.profiling_ms << '/' << cold.execution_ms << " ms"
            << " cached(total/scheduler/exec)="
            << cached.total_ms << '/' << cached.scheduler_ms << '/'
            << cached.execution_ms << " ms"
