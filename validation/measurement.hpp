...
namespace smart::validation
{
    ...
    inline MeasurementStatistics summarize(
        ...
        double confidence_z = 1.96)
    {
        ...
        std::size_t trim = samples.size() >= 7
            ? samples.size() / 10
            : 0;
        if (trim * 2 >= samples.size())
            trim = 0;
        const auto trimmed_begin = samples.begin() +
            static_cast<std::ptrdiff_t>(trim);
        const auto trimmed_end = samples.end() -
            static_cast<std::ptrdiff_t>(trim);
        result.trimmed_mean_ms = std::accumulate(
            trimmed_begin, trimmed_end, 0.0) /
        ...
        result.standard_deviation_ms = samples.size() > 1
            ? std::sqrt(squared /
        ...
        const double robust_sigma =
            result.median_absolute_deviation_ms > 0.0
                ? 1.4826 * result.median_absolute_deviation_ms
                : result.standard_deviation_ms;
        const double median_standard_error = samples.size() > 1
            ? 1.2533 * robust_sigma /
                std::sqrt(static_cast<double>(samples.size()))
            : 0.0;
        const double half_width = std::max(
            0.0, confidence_z * median_standard_error);
        result.confidence_low_ms = std::max(
            0.0, result.median_ms - half_width);
        result.confidence_high_ms = result.median_ms + half_width;
        return result;
    }

    inline bool confidence_intervals_overlap(
        const MeasurementStatistics& left,
    ...
    std::vector<MeasurementStatistics> measure_interleaved(
        ...
        Runner&& runner)
    {
        ...
        for (std::size_t round = 0;
             round < std::max<std::size_t>(1, config.measured_rounds);
             ++round)
        {
            std::shuffle(order.begin(), order.end(), random);
            for (std::size_t index : order)
        ...
        for (const auto& candidate_samples : samples)
            results.push_back(summarize(
                candidate_samples, config.confidence_z));
        return results;
    }
    ...
    inline RegretMetrics regret_metrics(
        const std::vector<double>& regrets)
    {
        ...
        result.worst_percent = *std::max_element(
