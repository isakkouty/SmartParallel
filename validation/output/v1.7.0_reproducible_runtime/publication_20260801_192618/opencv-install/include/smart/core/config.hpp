...
namespace detail
{
inline const Config*& active_config_override() noexcept
{
    static thread_local const Config* config = nullptr;
    return config;
}
}
...
// Production code reads through effective_config(). Explicit Runtime contexts
// install an immutable configuration snapshot while the legacy free-function
// path continues to observe global_config().
inline const Config& effective_config() noexcept
{
    const Config* configured = detail::active_config_override();
    return configured == nullptr ? global_config() : *configured;
}
} // namespace smart
