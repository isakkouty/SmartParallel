                                             ...
                                             bool work_executed = true)
{
    ...
    report.requested_engine = effective_config().execution_engine;
    report.requested_worker_budget = effective_config().nested_root_concurrency_budget == 0
        ? hardware_threads() : effective_config().nested_root_concurrency_budget;
    if (work_executed)
    {
