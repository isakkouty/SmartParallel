...
template <typename InputIterator, typename T, typename BinaryOperation>
T parallel_reduce(const ExecutionContext& context,
                  InputIterator first,
                  InputIterator last,
                  T init,
                  BinaryOperation binary_operation,
...
{
    if (numerical_options.policy != NumericalPolicy::Fast)
        throw std::invalid_argument(
            "SmartParallel generic parallel_reduce supports only Fast policy; use named dot/norm reductions for reproducible or accurate execution");
    ...
    return parallel_reduce(first, last, std::move(init), std::move(binary_operation));
}
...
T parallel_reduce(const ExecutionContext& context,
                  ...
                  NumericalOptions numerical_options = {})
{
    return parallel_reduce(context, first, last, std::move(init), std::plus<>{}, numerical_options);
}
