...
struct ExecutionContext
{
    ...
    // v1.7 Runtime ownership. A context keeps both the immutable scheduler
    // configuration snapshot and the Runtime adaptive/profile state alive.
    std::shared_ptr<const Config> runtime_config;
...
class ExecutionContextScope
{
  public:
    explicit ExecutionContextScope(const ExecutionContext& context) noexcept
        : previous_(active_execution_context()),
          previous_config_(active_config_override()),
          previous_adaptive_state_(bind_runtime_adaptive_state(
              context.runtime_state, context.legacy_global_adaptive_state))
    {
        active_execution_context() = &context;
        if (context.runtime_config)
            active_config_override() = context.runtime_config.get();
    }

    ~ExecutionContextScope()
    {
        active_config_override() = previous_config_;
        active_execution_context() = previous_;
        restore_runtime_adaptive_state(previous_adaptive_state_);
    }
  ...
  private:
    ...
    const Config* previous_config_ = nullptr;
    void* previous_adaptive_state_ = nullptr;
};
} // namespace detail
                                      ...
                                      const ExecutionContext& parent) noexcept
{
    ...
    if (!context.runtime_config && has_parent)
        context.runtime_config = parent.runtime_config;
    if (!context.runtime_state && has_parent)
        context.runtime_state = parent.runtime_state;
    if (has_parent)
    {
        context.legacy_global_adaptive_state = parent.legacy_global_adaptive_state;
