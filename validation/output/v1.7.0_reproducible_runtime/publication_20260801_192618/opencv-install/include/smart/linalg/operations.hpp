...
template <typename T>
void axpy_impl(smart::data::VectorView<T> y,
          T alpha,
...
template <typename T>
T dot_impl(smart::data::VectorView<const T> x,
...
template <typename T>
T norm_impl(smart::data::VectorView<const T> x,
       NumericalOptions options = {})
{
    ...
    if (options.policy == NumericalPolicy::Reproducible)
        smart::detail::require_numerical_capability(
            smart::detail::native_pairwise_reduction_capabilities, options.policy);
    T squared = dot_impl<T>(x, x, options);
    ...
    NumericalOptions options)
{
    ...
    descriptor.extents = {x.extent(0)};
    descriptor.strides = {x.stride(0)};
    descriptor.layout = x.is_contiguous() ? "contiguous" : "strided";
    if (options.policy == NumericalPolicy::Fast)
    {
...
template <typename T>
void axpy(const ExecutionContext& context,
          smart::data::VectorView<T> y,
          T alpha,
          smart::data::VectorView<const T> x,
          NumericalOptions options = {})
{
    auto descriptor = detail_linalg::vector_descriptor<T>("smart.linalg.axpy", x, options);
    descriptor.strides.push_back(y.stride(0));
    descriptor.in_place = true;
    descriptor.semantic_constants = "alpha=" + smart::detail::floating_value_identity(&alpha, sizeof(alpha));
    if (options.policy != NumericalPolicy::Fast)
    {
        descriptor.expected_evaluation_order = "canonical_pointwise";
        descriptor.expected_accumulation = "fixed_pointwise_expression";
        descriptor.expected_canonical_plan = smart::detail::canonical_pointwise_plan_v1;
    }
    ...
    axpy_impl(y, alpha, x, options);
    smart::detail::complete_semantic_operation(
        prepared, detail_linalg::selected_native_route());
}

template <typename T>
T dot(const ExecutionContext& context,
      smart::data::VectorView<const T> x,
      smart::data::VectorView<const T> y,
      NumericalOptions options = {})
{
    auto descriptor = detail_linalg::vector_descriptor<T>("smart.linalg.dot", x, options);
    descriptor.strides.push_back(y.stride(0));
    if (options.policy == NumericalPolicy::Reproducible)
    {
        descriptor.expected_evaluation_order = "canonical_deterministic";
        descriptor.expected_accumulation = "canonical_pairwise";
        descriptor.expected_canonical_plan = smart::detail::canonical_pairwise_plan_v1;
    }
    else if (options.policy == NumericalPolicy::Accurate)
    {
        descriptor.expected_evaluation_order = "canonical_deterministic";
        descriptor.expected_accumulation = "compensated";
        descriptor.expected_canonical_plan = smart::detail::compensated_plan_v1;
    }
    auto prepared = smart::detail::prepare_semantic_operation(context, std::move(descriptor));
    smart::detail::SemanticOperationScope scope(prepared);
    T result = dot_impl(x, y, options);
    smart::detail::complete_semantic_operation(
        prepared, detail_linalg::selected_native_route());
    return result;
}
...
template <typename T>
T norm(const ExecutionContext& context,
       smart::data::VectorView<const T> x,
       NumericalOptions options)
{
    auto descriptor = detail_linalg::vector_descriptor<T>("smart.linalg.norm", x, options);
    if (options.policy == NumericalPolicy::Reproducible)
    {
        descriptor.expected_evaluation_order = "canonical_deterministic";
        descriptor.expected_accumulation = "canonical_pairwise";
    ...
    {
        descriptor.expected_evaluation_order = "canonical_deterministic";
        descriptor.expected_accumulation = "scaled_sum_of_squares";
        descriptor.expected_canonical_plan = smart::detail::scaled_sumsq_plan_v1;
    }
    auto prepared = smart::detail::prepare_semantic_operation(context, std::move(descriptor));
    smart::detail::SemanticOperationScope scope(prepared);
    T result = norm_impl(x, options);
    smart::detail::complete_semantic_operation(
        prepared, detail_linalg::selected_native_route());
    return result;
}
...
void axpy(const ExecutionContext& context,
          ...
          smart::data::VectorView<const T> x)
{
    axpy(context, y, alpha, x, NumericalOptions{
...
T dot(const ExecutionContext& context,
      ...
      smart::data::VectorView<const T> y)
{
    return dot(context, x, y, NumericalOptions{
        execution_context_default_numerical_policy(context)});
}
...
T norm(const ExecutionContext& context,
       smart::data::VectorView<const T> x)
{
    return norm(context, x, NumericalOptions{
        execution_context_default_numerical_policy(context)});
}
...
void axpy(smart::data::VectorView<T> y,
          ...
          NumericalOptions options = {})
{
    axpy(smart::implicit_execution_context(), y, alpha, x, options);
}
...
T dot(smart::data::VectorView<const T> x,
      ...
      NumericalOptions options = {})
{
    return dot(smart::implicit_execution_context(), x, y, options);
}
...
T norm(smart::data::VectorView<const T> x,
       NumericalOptions options = {})
{
    return norm(smart::implicit_execution_context(), x, options);
}
