  ...
  public:
                        ...
                        std::uint64_t expected_cache_epoch = 0)
    {
        ...
        const double nested_blend = std::clamp(
            effective_config().parallel_for_profile_nested_evidence_blend, 0.0, 1.0);
        const double nested_threshold = std::clamp(
            effective_config().parallel_for_profile_nested_evidence_threshold, 0.0, 1.0);
  ...
  private:
    ...
    bool make_room_for(const FunctionProfileKey& incoming_key)
    {
        const std::size_t maximum = runtime_limits::bounded_limit(
            effective_config().parallel_for_profile_cache_max_entries,
...
{
    if (auto* cache = detail::active_runtime_function_profile_cache())
        return *cache;
