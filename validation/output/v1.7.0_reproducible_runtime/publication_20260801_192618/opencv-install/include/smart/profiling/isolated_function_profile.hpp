...
FunctionProfile profile_container_on_copies(const Container& container,
                                            ...
                                            const FunctionProfiler::Config& config)
{
    ...
    {
        profile.unavailable_reason = FunctionProfileUnavailableReason::ValueNotCopyConstructible;
        return profile;
    }
    else if constexpr (!std::is_copy_constructible_v<Callable>)
    {
        profile.unavailable_reason = FunctionProfileUnavailableReason::CallableNotCopyConstructible;
        return profile;
    }
    else if constexpr (!std::is_invocable_v<Callable&, ValueType&>)
    {
        profile.unavailable_reason = FunctionProfileUnavailableReason::CallableNotInvocableOnCopy;
        return profile;
    }
    else
    {
        ...
        {
            profile.sampling_mode = FunctionProfileSamplingMode::IsolatedCopies;
        }
...
FunctionProfile profile_pair_on_copies(const ContainerA& a,
                                       const ContainerB& b,
                                       const Function& function,
                                       const FunctionProfiler::Config& config)
{
    ...
    if constexpr (!std::is_copy_constructible_v<ValueTypeA>
    ...
    else if constexpr (!std::is_invocable_v<Callable&, ValueTypeA&, ValueTypeB&>)
    {
        profile.unavailable_reason = FunctionProfileUnavailableReason::CallableNotInvocableOnCopy;
        return profile;
    }
    ...
    {
        const std::size_t size_b = static_cast<std::size_t>(b.size());

        const SizeCalculation total_calculation =
            saturating_multiply(static_cast<std::size_t>(a.size()), size_b);

        if (total_calculation.saturated)
        {
            profile.unavailable_reason = FunctionProfileUnavailableReason::IterationCountOverflow;
            return profile;
        }
        ...
        profile = profiler.profile_index_range(
            ...
            [&](std::size_t flat_index)
            {
                ...
                std::invoke(profiling_function, sample_a, sample_b);
            },
            config);
