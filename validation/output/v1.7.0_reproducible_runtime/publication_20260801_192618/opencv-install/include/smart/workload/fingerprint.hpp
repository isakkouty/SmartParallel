...
inline std::size_t fingerprint_bucket(std::size_t value)
{
    ...
    while (bucket < value && bucket <= (static_cast<std::size_t>(-1) >> 1))
    {
        bucket <<= 1;
    }
...
inline std::size_t fingerprint_real_bucket(double value)
{
    ...
    const std::size_t mantissa_bucket =
        static_cast<std::size_t>(std::max(0.0, std::min(15.0, std::floor(mantissa * 16.0))));
    const std::size_t exponent_bucket = static_cast<std::size_t>(exponent + 2048);
...
inline WorkloadFingerprint fingerprint(const Workload& workload, const FunctionProfile* profile)
{
    std::size_t seed = 0;
    ...
    if (profile != nullptr && profile->available)
    {
        ...
        fingerprint_combine(seed, fingerprint_real_bucket(profile->coefficient_of_variation + 1.0));
        fingerprint_combine(seed, fingerprint_real_bucket(profile->tail_ratio));
        fingerprint_combine(seed, fingerprint_real_bucket(profile->regional_cost_ratio));
    }
    else
    {
    ...
    for (const Dimension& dimension : workload.dimensions)
    {
        const std::size_t access_bits =
            ...
            | (dimension.contiguous_known ? 0x10 : 0) | (dimension.contiguous ? 0x20 : 0)
            | (dimension.random_access_known ? 0x40 : 0) | (dimension.random_access ? 0x80 : 0);
        fingerprint_combine(access_seed, access_bits);
        fingerprint_combine(
            stride_seed, dimension.stride_known ? fingerprint_bucket(dimension.stride_bytes) : 0);
    }
    result.access_pattern_bucket = access_seed;
    result.stride_bucket = stride_seed;

    const std::size_t effective_l3 = hw.l3_cache_size > 0 ? hw.l3_cache_size : 32 * 1024 * 1024;
    result.cache_pressure_bucket =
        fingerprint_real_bucket(static_cast<double>(represented_bytes)
                                    / static_cast<double>(std::max<std::size_t>(1, effective_l3))
                                + 1.0);
    ...
    if (profile != nullptr && profile->available)
    {
        ...
        result.variation_bucket = fingerprint_real_bucket(
            1.0 + profile->coefficient_of_variation + std::max(0.0, profile->tail_ratio - 1.0)
            + std::max(0.0, profile->regional_cost_ratio - 1.0));
        result.profile_shape_bucket = 0;
        fingerprint_combine(result.profile_shape_bucket,
