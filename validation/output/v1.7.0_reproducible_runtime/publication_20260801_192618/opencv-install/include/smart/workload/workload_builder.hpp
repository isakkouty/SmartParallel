...
template <typename T>
struct has_index_operator<T, std::void_t<decltype(std::declval<T&>()[std::size_t{}])>>
    : std::true_type
{
};
...
template <typename T>
struct has_compatible_data<T, std::void_t<decltype(std::data(std::declval<T&>()))>>
...
Dimension describe_dimension(Container& container)
{
    ...
    constexpr bool proxy_reference =
        !std::is_same_v<std::remove_cv_t<std::remove_reference_t<Reference>>, ValueType>;
...
struct WorkloadBuilder
{
    ...
    static Workload container(Container& container)
    {
        ...
        workload.dimensions.push_back(detail::describe_dimension(container));
        return workload;
    }
    ...
    static Workload pair_container(ContainerA& a, ContainerB& b)
    {
        ...
        const std::size_t size_a = static_cast<std::size_t>(a.size());
        const std::size_t size_b = static_cast<std::size_t>(b.size());

        const SizeCalculation iteration_count = saturating_multiply(size_a, size_b);
