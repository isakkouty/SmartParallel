...
{
  public:
    MemoryFeatures build(const WorkloadAnalysis& analysis,
                         ...
                         const ExecutionHints& hints) const
    {
        ...
        result.contiguous_ratio =
            contiguous_known == 0
                ? 0.0
                : static_cast<double>(contiguous_count) / static_cast<double>(contiguous_known);
        const double structural_random =
            random_known == 0
                ? 0.0
                : static_cast<double>(random_count) / static_cast<double>(random_known);

        const double hint_confidence =
            hints.available ? std::clamp(hints.feature_confidence, 0.0, 1.0) : 0.0;
        result.random_access_ratio =
            hints.available ? (1.0 - hint_confidence) * structural_random
                                  + hint_confidence * std::clamp(hints.memory_randomness, 0.0, 1.0)
                            : structural_random;
        result.arithmetic_intensity =
            hints.available ? std::clamp(hints.arithmetic_intensity, 0.0, 1.0) : 0.0;
        result.memory_level_parallelism =
            hints.available ? std::clamp(hints.estimated_memory_level_parallelism / 8.0, 0.0, 1.0)
                            : 0.0;
        result.dependency_score =
            hints.available ? std::clamp(std::max(hints.dependency_depth,
        ...
        result.bytes_touched_per_iteration =
            hints.available && hints.bytes_touched_per_iteration > 0.0
                ? hints.bytes_touched_per_iteration
                : represented_per_iteration;
        result.l3_pressure = std::max(model.l3_pressure,
        ...
        const bool pointer_chasing =
            hints.available
            && (hints.dependency_depth > 0.0 || hints.dependent_memory_accesses_per_iteration > 0.0)
            && result.random_access_ratio >= 0.45;
        const bool random_access = result.random_access_ratio >= 0.55 || node_or_segmented;
        const bool sequential =
            result.contiguous_ratio >= 0.70 && result.random_access_ratio < 0.30;
        ...
        result.cache_resident = result.working_set_tier == WorkingSetTier::L1Resident
                                || result.working_set_tier == WorkingSetTier::L2Resident
                                || result.working_set_tier == WorkingSetTier::L3Resident;
        result.large_record =
            analysis.objects_are_large || result.bytes_touched_per_iteration >= 128.0;
        result.latency_sensitive = result.access_pattern == MemoryAccessPattern::Random
                                   || result.access_pattern == MemoryAccessPattern::PointerChasing
                                   || result.dependency_score >= 0.25;
        result.bandwidth_sensitive = result.access_pattern == MemoryAccessPattern::Sequential
                                     && result.working_set_tier == WorkingSetTier::MemoryResident
                                     && result.arithmetic_intensity <= 0.45;

        const double locality = std::clamp(
            0.65 * result.contiguous_ratio + 0.35 * (1.0 - result.random_access_ratio), 0.0, 1.0);
        const double residence =
            result.cache_resident ? 1.0
        ...
        if (analysis.structural.represented_input_bytes > 0)
            evidence += 0.10;
        if (strided_count > 0 || node_or_segmented)
            evidence += 0.08;
        if (hints.available)
            evidence += 0.18 * hint_confidence;
