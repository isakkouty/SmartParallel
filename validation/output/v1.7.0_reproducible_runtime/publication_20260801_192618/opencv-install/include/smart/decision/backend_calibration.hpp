  ...
  private:
                                                ...
                                                ExecutionEngineType second) const noexcept
    {
        ...
        const double hysteresis = std::max(
            0.0, effective_config().parallel_for_backend_calibration_hysteresis_percent) / 100.0;
    ...
    Entry& find_or_create_unlocked(const FunctionProfileKey& key)
    {
        ...
        const std::size_t maximum = runtime_limits::bounded_limit(
            effective_config().parallel_for_backend_calibration_max_entries,
...
{
    if (auto* cache = detail::active_runtime_backend_calibration_cache())
        return *cache;
