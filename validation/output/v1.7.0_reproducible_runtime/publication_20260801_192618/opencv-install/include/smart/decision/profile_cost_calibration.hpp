...
{
  public:
    ProfileCostCalibrationResult evaluate(const FunctionProfile& profile,
                                          std::size_t iterations) const
    {
        ...
        result.mean_ms =
            finite_positive(profile.avg_ms_per_iteration) ? profile.avg_ms_per_iteration : 0.0;

        double per_iteration = result.baseline_ms;
        if (finite_positive(per_iteration) && finite_positive(result.mean_ms))
        {
            ...
            const double variation =
                std::clamp(profile.coefficient_of_variation * 0.30
            ...
            per_iteration =
                per_iteration * (1.0 - result.mean_weight) + bounded_mean * result.mean_weight;
            result.mean_blended = result.mean_weight > 1.0e-12;
        }

        if (!finite_positive(per_iteration) && finite_positive(profile.estimated_total_work_ms)
            && iterations > 0)
        {
            per_iteration = profile.estimated_total_work_ms / static_cast<double>(iterations);
        }
        ...
        result.total_work_ms = std::max(0.0, profile.estimated_setup_cost_ms)
