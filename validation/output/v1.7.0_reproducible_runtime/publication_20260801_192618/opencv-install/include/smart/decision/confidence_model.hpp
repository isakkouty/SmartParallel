...
{
  public:
    static ConfidenceAssessment assess_candidate(const PlanCostEstimate& candidate)
    {
        ...
        const double hierarchical_confidence =
            std::clamp(candidate.hierarchical_residual_confidence, 0.0, 1.0);
        result.residual_confidence =
            exact_residual_confidence > 0.0 || hierarchical_confidence > 0.0
                ? std::max(exact_residual_confidence, hierarchical_confidence)
                : 0.50;

        if (candidate.experience_rank_used)
        {
            result.history_confidence = std::clamp(candidate.historical_evidence_confidence
        ...
        result.similarity_confidence =
            candidate.similarity_rank_used
                ? std::clamp(candidate.similarity_rank_confidence, 0.0, 1.0)
                : 0.60;
        ...
        result.agreement_confidence = std::clamp(
            std::exp(-disagreement / std::max(0.05, effective_config().confidence_disagreement_scale)),
            0.0,
            1.0);
        ...
        const double weighted_log = 0.30 * std::log(std::max(0.05, result.profile_confidence))
        ...
        result.combined_confidence = std::clamp(
            std::exp(weighted_log), effective_config().minimum_candidate_model_confidence, 1.0);
        result.uncertainty_penalty = 1.0 - result.combined_confidence;
        return result;
    }
    ...
    static void apply(std::vector<PlanCostEstimate>& candidates)
    {
        ...
        for (PlanCostEstimate& candidate : candidates)
        {
            ...
            if (effective_config().enable_confidence_aware_ranking
                && (!effective_config().enable_risk_aware_ranking
                    || candidate.risk_adjusted_total_ms <= 0.0))
            {
                ...
                candidate.ranking_score *= 1.0
                                           + effective_config().maximum_confidence_risk_penalty
        ...
        const double margin = best > 0.0 ? std::max(0.0, second - best) / best : 0.0;
        const double margin_confidence = std::clamp(
            margin / std::max(0.01, effective_config().full_confidence_score_margin), 0.0, 1.0);

        for (PlanCostEstimate& candidate : candidates)
        {
            candidate.decision_margin_confidence = margin_confidence;
            candidate.confidence =
                std::clamp(candidate.model_confidence * (0.75 + 0.25 * margin_confidence),
                           effective_config().minimum_candidate_model_confidence,
                           1.0);
        }
    }
};
