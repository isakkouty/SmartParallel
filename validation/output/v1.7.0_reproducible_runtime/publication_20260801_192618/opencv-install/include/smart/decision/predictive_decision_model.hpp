...
class PredictiveDecisionModel
{
  public:
                                     ...
                                     const ExecutionHints* hints = nullptr) const
    {
        ...
        const RuntimeScalingResult scaling =
            effective_config().enable_learned_runtime_scaling
                ? RuntimeScalingPolicy().evaluate(
                      linear_useful_work_ms, workload.iterations, *profile, family_classification)
                : RuntimeScalingResult{};

        const double useful_work_ms = effective_config().enable_learned_runtime_scaling
        ...
        for (std::size_t workers : worker_counts)
        {
            ...
            if (effective_config().enable_static_thread_auto_candidates)
            {
        ...
        if (effective_config().enable_family_specific_calibration)
        {
        ...
        if (effective_config().enable_memory_bandwidth_calibration)
        {
        ...
        if (effective_config().enable_memory_access_calibration)
        {
        ...
        if (effective_config().enable_memory_aware_feature_model)
        {
        ...
        if (effective_config().enable_analytical_runtime_model)
        {
        ...
        if (effective_config().enable_hierarchical_residual_learning)
        {
            for (PlanCostEstimate& candidate : result.candidates)
                global_hierarchical_residual_learner().apply(candidate);
        }

        if (effective_config().enable_residual_correction)
        {
        ...
        else if (effective_config().enable_prediction_calibration)
        {
        ...
        if (effective_config().enable_confidence_model)
  ...
  private:
    static std::vector<std::size_t> adaptive_worker_counts(std::size_t maximum_workers)
    {
        ...
        if (!effective_config().enable_adaptive_execution_candidates)
        {
        ...
        const std::size_t minimum_workers =
            std::max<std::size_t>(2, effective_config().minimum_adaptive_workers);
                                                         ...
                                                         const FunctionProfile& profile)
    {
        ...
        if (!effective_config().enable_adaptive_execution_candidates
            || !effective_config().enable_chunk_neighborhood_candidates || iterations <= 1)
        {
            return chunks;
        }
                                             ...
                                             const WorkloadFingerprint& fingerprint)
    {
        ...
        if (entry == nullptr
            || entry->prediction_sample_count < effective_config().minimum_calibration_samples
            || entry->confidence < effective_config().minimum_calibration_confidence)
        {
            return;
        }
    ...
    static double scheduling_overhead(ExecutionEngineType engine,
                                      ...
                                      const FunctionProfile& profile)
    {
        if (effective_config().enable_machine_runtime_calibration)
        {
                                           ...
                                           std::size_t workers)
    {
        ...
        if (effective_config().enable_hardware_aware_prediction)
        {
            ...
            {
                factor /= 1.0
                          + effective_config().hardware_l2_pressure_penalty
                                * std::log2(context.l2_pressure_per_worker + 1.0);
            }
            factor *= 1.0 - effective_config().hardware_numa_penalty * context.numa_pressure;
        }
                       ...
                       double confidence)
    {
        ...
        if (effective_config().enable_machine_runtime_calibration)
        {
            ...
            estimate.machine_calibration_authority =
                std::clamp(raw_authority,
                           effective_config().minimum_machine_calibration_authority,
                           effective_config().maximum_machine_calibration_authority);
        }
        ...
        const double calibration_authority = effective_config().enable_machine_runtime_calibration
                                                 ? estimate.machine_calibration_authority
                                                 : 0.0;
        ...
        estimate.machine_calibration_used = effective_config().enable_machine_runtime_calibration;
