...
{
  public:
    RuntimeScalingResult evaluate(double linear_total_work_ms,
                                  ...
                                  const WorkloadFamilyClassification& family) const
    {
        ...
        if (!std::isfinite(linear_total_work_ms) || linear_total_work_ms <= 0.0 || iterations == 0
            || profile.callback_invocations == 0)
        {
            return result;
        }

        const double sampled =
            static_cast<double>(std::min<std::size_t>(iterations, profile.callback_invocations));
        ...
        if (profile.spatial_observations_available && profile.local_median_ms_per_iteration > 0.0
            && profile.distributed_median_ms_per_iteration > 0.0)
        {
            const double spatial_ratio = std::clamp(profile.distributed_median_ms_per_iteration
        ...
        const double sample_evidence =
            std::clamp(static_cast<double>(profile.measured_batches) / 12.0, 0.0, 1.0);
        const double family_confidence = std::clamp(family.confidence, 0.0, 1.0);
        result.confidence =
            std::clamp(reliability * sample_evidence
                           * (0.35 + 0.40 * family_confidence + 0.25 * shape_confidence),
                       0.0,
                       1.0);

        const double learned_exponent =
            prior_exponent * (1.0 - shape_confidence) + observed_exponent * shape_confidence;
        result.scaling_exponent =
            std::clamp(1.0 + (learned_exponent - 1.0) * result.confidence, 0.90, 1.08);

        const double raw_factor = std::pow(ratio, result.scaling_exponent - 1.0);
        result.correction_factor = std::clamp(raw_factor, 0.72, 1.35);
        result.scaled_total_work_ms = linear_total_work_ms * result.correction_factor;
