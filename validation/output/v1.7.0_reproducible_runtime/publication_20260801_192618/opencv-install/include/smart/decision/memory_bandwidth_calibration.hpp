...
{
  public:
    MemoryBandwidthCalibrationResult evaluate(const ExecutionPlan& plan,
                                              ...
                                              const PerformanceModel& model) const
    {
        ...
        if (classification.family != WorkloadFamily::StreamingMemory
            || classification.confidence <= 0.0 || analysis.structural.logical_iterations == 0)
        {
            return result;
        }

        const double confidence = std::clamp(classification.confidence, 0.0, 1.0);
        const double l3_pressure = std::max(model.l3_pressure,
                                            analysis.structural.cache_ratios_available
                                                ? analysis.structural.l3_residency_ratio
                                                : 0.0);

        result.bytes_per_iteration =
            model.function.available && model.function.bytes_touched_per_iteration > 0.0
        ...
        result.large_record_stream =
            analysis.objects_are_large || result.bytes_per_iteration >= 128.0;

        const double pressure_strength =
            std::clamp(std::log2(std::max(1.0, l3_pressure)) / 5.0, 0.0, 1.0);
        const double record_strength =
            std::clamp(std::log2(std::max(1.0, result.bytes_per_iteration / 64.0)) / 4.0, 0.0, 1.0);
        result.saturation_strength =
            std::clamp(0.72 * pressure_strength + 0.28 * record_strength, 0.0, 1.0);
        ...
        const double saturation_fraction = 0.78 - 0.43 * result.saturation_strength;
        result.estimated_saturation_workers =
            std::max<std::size_t>(2,
                                  static_cast<std::size_t>(std::ceil(static_cast<double>(physical)
                                                                     * saturation_fraction)));
        result.estimated_saturation_workers =
            std::min(result.estimated_saturation_workers, logical);
        ...
        if (plan.parallel)
        {
            ...
            {
                result.worker_excess =
                    std::clamp(static_cast<double>(workers - result.estimated_saturation_workers)
                                   / static_cast<double>(std::max<std::size_t>(
                                       1, logical - result.estimated_saturation_workers)),
                               0.0,
                               1.0);
            }
            ...
            family_factor +=
                ...
                + 0.42 * result.worker_excess * (0.45 + 0.55 * result.saturation_strength);

            if (result.large_record_stream)
            {
                const double worker_pressure = std::clamp(
                    static_cast<double>(workers) / static_cast<double>(physical), 0.0, 2.0);
    ...
    MemoryBandwidthCalibrationResult apply(PlanCostEstimate& estimate,
                                           const WorkloadFamilyClassification& classification,
                                           const WorkloadAnalysis& analysis,
                                           const PerformanceModel& model) const
    {
        const MemoryBandwidthCalibrationResult result =
        ...
        estimate.memory_bandwidth_saturation_strength = result.saturation_strength;
        estimate.memory_bandwidth_worker_excess = result.worker_excess;
        estimate.memory_bandwidth_bytes_per_iteration = result.bytes_per_iteration;
        estimate.memory_bandwidth_saturation_workers = result.estimated_saturation_workers;
