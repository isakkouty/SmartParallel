...
{
  public:
    std::optional<DecisionReport> apply_report(const DecisionContext& context,
                                               const ExecutionHints& hints) const
    {
        if (!context.analysis.is_small)
            return std::nullopt;

        if (context.function_profile && context.function_profile->available
  ...
  public:
    ...
    DecisionReport apply_report(const DecisionContext& context, const ExecutionHints& hints) const
    {
        ...
        if (!(context.function_profile && context.function_profile->available
            ...
            && !hints.available)
        {
            if (context.analysis.iterations <= effective_config().cheap_workload_sequential_threshold)
            {
