...
{
  public:
    ResidualFeatureVector build(const Workload& workload,
                                ...
                                double scheduling_overhead_ms) const
    {
        ...
        const double physical = static_cast<double>(std::max<std::size_t>(
            1,
            model.hardware.physical_cores == 0 ? model.hardware.logical_threads
                                               : model.hardware.physical_cores));
        const double logical =
            static_cast<double>(std::max<std::size_t>(1, model.hardware.logical_threads));

        const double workers =
            plan.parallel ? static_cast<double>(std::max<std::size_t>(1, plan.job_count)) : 1.0;
        ...
        const double chunks_per_worker = iterations / std::max(1.0, chunk * workers);

        const double represented_bytes =
            static_cast<double>(analysis.structural.represented_input_bytes);
        const double effective_working_set =
            represented_bytes
            + static_cast<double>(
                hints != nullptr && hints->available ? hints->external_working_set_bytes : 0);
        const double l3_bytes =
            static_cast<double>(std::max<std::size_t>(1, model.hardware.l3_cache_size));
        const double l3_ratio = effective_working_set / l3_bytes;
        const double bytes_per_iteration =
            hints != nullptr && hints->available && hints->bytes_touched_per_iteration > 0.0
                ? hints->bytes_touched_per_iteration
                : represented_bytes / iterations;

        const double per_iteration_ms = useful_work_ms / std::max(1.0, iterations);
        ...
        x[16] = !plan.parallel || plan.strategy == ExecutionStrategy::Sequential ? 1.0 : 0.0;
        x[17] = plan.parallel && plan.engine == ExecutionEngineType::ThreadPool ? 1.0 : 0.0;
        x[18] = plan.parallel && plan.engine == ExecutionEngineType::StaticThread ? 1.0 : 0.0;
        x[19] = plan.parallel && plan.engine == ExecutionEngineType::OneTbb ? 1.0 : 0.0;
        ...
        const double profile_confidence =
        ...
        result.feature_confidence =
            clamp(0.45 * profile_confidence + 0.25 * structural_confidence
                      + 0.20 * std::max(0.25, classification.confidence) + 0.10 * hint_confidence,
                  0.0,
                  1.0);
  ...
  private:
    ...
    {
        return clamp(std::log2(std::max(1.0, value)) / std::max(1.0, denominator), 0.0, 1.5);
    }
    ...
    static double signed_log2_ratio(double value, double scale)
    {
        if (!std::isfinite(value) || value <= 0.0)
            return -1.0;
        return clamp(std::log2(value) / std::max(1.0, scale), -1.0, 1.0);
    }

    using HintMember = double ExecutionHints::*;

    static double hint_value(const ExecutionHints* hints, HintMember member)
    {
    ...
    static double dependency_strength(const ExecutionHints* hints)
    {
        ...
        const double mlp = hints->estimated_memory_level_parallelism > 0.0
                               ...
                               : 1.0;
        return clamp(
            std::log1p(accesses + depth) / std::log(9.0) / std::max(1.0, std::sqrt(mlp)), 0.0, 1.0);
    }
};
} // namespace smart
