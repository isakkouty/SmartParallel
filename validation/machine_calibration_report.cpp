...
int main()
{
    try
    {
        ...
        const std::filesystem::path output_path = output_dir / "machine_calibration.csv";
        ...
        csv << "engine,workers,base_overhead_ms,log_slope_ms,"
        ...
        const smart::BackendRuntimeCalibration& calibration = smart::backend_runtime_calibration();
        ...
        for (const smart::BackendRuntimeCalibrationPoint& point : calibration.points)
        {
            const auto overhead = [&](std::size_t chunks)
            {
                ...
                return smart::calibrated_backend_overhead_ms(
                    point.engine, chunks, point.workers, 1);
            };

            csv << smart::engine_name(point.engine) << ',' << point.workers << ','
                << point.base_overhead_ms << ',' << point.log_slope_ms << ','
                << point.overhead_relative_uncertainty << ',' << point.cheap_small_speedup << ','
                << point.cheap_large_speedup << ',' << point.compute_speedup << ','
                << point.streaming_speedup << ',' << point.speedup_relative_uncertainty << ','
                << overhead(1) << ',' << overhead(8) << ',' << overhead(64) << ',' << overhead(512)
                << '\n';
        }

        std::cout << "==== SmartParallel Machine Calibration Report ====\n"
                  << "Measured calibration: " << (calibration.measured ? "yes" : "no") << '\n'
    ...
    {
        std::cerr << "Machine calibration report failed: " << error.what() << '\n';
        return 1;
    }
}
