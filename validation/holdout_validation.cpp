...
ValidationRow summarize_case(const std::string& suite,
                             ...
                             std::ofstream& candidates_csv)
{
    ...
    const smart::ranking::DecisionContext decision_context =
        smart::ranking::make_decision_context(logical_iterations, &profile, hints);
    ...
    for (const MeasuredCandidate& item : measured)
    {
        ...
        candidates_csv << suite << ',' << case_name << ',' << logical_iterations << ','
    ...
    row.exact_winner = same_plan(prediction.recommended_plan, best_actual->estimate.plan);
    row.within_tolerance =
        predicted_winner->actual_ms <= best_actual->actual_ms * (1.0 + near_optimal_tolerance)
        || smart::validation::confidence_intervals_overlap(predicted_winner->timing,
                                                           best_actual->timing);
...
void clustered_work(int& value)
{
    ...
    const bool expensive_cluster = (position % 4096u) >= 3072u;
...
void sparse_like_work(int& value)
{
    ...
    std::uint32_t index =
        static_cast<std::uint32_t>(value) % static_cast<std::uint32_t>(pointer_chain.size());
...
void prepare_pointer_chain(std::size_t size)
{
    ...
    {
        pointer_chain[i] =
            static_cast<std::uint32_t>((static_cast<std::uint64_t>(i) * multiplier + 97u) % size);
    }
}

ValidationRow run_int_case(const IntCase& definition, std::ofstream& candidates_csv)
{
    std::vector<int> values(definition.size);
    definition.reset(values);

    const smart::Workload workload = smart::WorkloadBuilder::container(values);
    const smart::WorkloadAnalysis analysis = smart::WorkloadAnalyzer().analyze(workload);
    const smart::FunctionProfile profile =
    ...
    {
        hints = smart::pointer_chasing(pointer_chain.size() * sizeof(std::uint32_t), 96.0);
    }
    ...
    const smart::PredictiveDecisionResult prediction = smart::PredictiveDecisionModel().predict(
        workload, analysis, &profile, hints.available ? &hints : nullptr);
    ...
    return summarize_case(definition.suite,
                          definition.name,
                          workload.iterations,
                          prediction,
                          measured,
...
void reset_large(std::vector<LargeRecord>& values)
{
    for (std::size_t i = 0; i < values.size(); ++i)
    {
        ...
        {
            values[i].words[j] = static_cast<std::uint64_t>(i * 131u + j * 17u + 3u);
        }
    }
}
...
ValidationRow run_large_case(const char* name, std::size_t size, std::ofstream& candidates_csv)
{
    ...
    const smart::WorkloadAnalysis analysis = smart::WorkloadAnalyzer().analyze(workload);
    const smart::FunctionProfile profile =
        smart::profile_container_on_copies(values, large_record_work, profile_config());
    const smart::PredictiveDecisionResult prediction =
        smart::PredictiveDecisionModel().predict(workload, analysis, &profile);
    ...
    measurement_config.random_seed ^= static_cast<std::uint64_t>(size) << 1u;
    const auto timings = smart::validation::measure_interleaved(
        measured.size(),
        measurement_config,
        [&](std::size_t candidate_index, bool)
    ...
    return summarize_case("large_object",
                          name,
                          workload.iterations,
                          prediction,
                          measured,
...
ValidationRow run_pair_case(const char* name,
                            ...
                            std::ofstream& candidates_csv)
{
    ...
    const smart::Workload workload = smart::WorkloadBuilder::pair_container(left, right);
    const smart::WorkloadAnalysis analysis = smart::WorkloadAnalyzer().analyze(workload);
    const smart::FunctionProfile profile =
        smart::profile_pair_on_copies(left, right, pair_profile_work, profile_config());
    const smart::PredictiveDecisionResult prediction =
        smart::PredictiveDecisionModel().predict(workload, analysis, &profile);
    ...
    smart::validation::MeasurementConfig measurement_config;
    measurement_config.warmup_rounds = 2;
    measurement_config.measured_rounds = workload.iterations <= 100'000 ? 9 : 7;
    measurement_config.random_seed ^= static_cast<std::uint64_t>(workload.iterations) << 2u;
    ...
    const auto timings = smart::validation::measure_interleaved(
        ...
        [&](std::size_t candidate_index, bool)
        {
            ...
            smart::execute_workload(workload,
                                    ...
                                    [&](std::size_t flat)
                                    {
                                        outputs[flat] = pair_kernel(left[flat / right_extent],
    ...
    return summarize_case(
        "pair", name, workload.iterations, prediction, measured, profile, nullptr, candidates_csv);
}
...
int main()
{
    ...
    try
    {
        ...
        if (error)
        {
            std::cerr << "Unable to determine working directory: " << error.message() << '\n';
            return 1;
        }

        const std::filesystem::path output_dir = root / "validation" / "output";
        ...
        const std::filesystem::path candidates_path = output_dir / "holdout_candidates.csv";
        const std::filesystem::path summary_path = output_dir / "holdout_summary.csv";
        const std::filesystem::path metrics_path = output_dir / "holdout_metrics.csv";
        ...
        summary_csv << "suite,case,logical_iterations,predicted_plan,"
        ...
        const std::vector<IntCase> int_cases = {
            {"memory", "memory_stream_50k", 50'000, memory_stream_work, reset_default},
            {"memory", "memory_stream_500k", 500'000, memory_stream_work, reset_default},
            {"memory", "memory_stream_5m", 5'000'000, memory_stream_work, reset_default},
            {"cache", "cache_resident_4k", 4'096, cache_resident_work, reset_default},
            {"cache", "cache_resident_64k", 65'536, cache_resident_work, reset_default},
            {"branch", "branch_heavy_512", 512, branch_heavy_work, reset_default},
            {"branch", "branch_heavy_8k", 8'192, branch_heavy_work, reset_default},
            {"tiny_heavy", "tiny_heavy_64", 64, tiny_heavy_work, reset_default},
            {"tiny_heavy", "tiny_heavy_256", 256, tiny_heavy_work, reset_default},
            {"clustered", "clustered_4k", 4'096, clustered_work, reset_position},
            {"clustered", "clustered_64k", 65'536, clustered_work, reset_position},
            {"sparse", "pointer_chase_4k", 4'096, sparse_like_work, reset_default},
            {"sparse", "pointer_chase_64k", 65'536, sparse_like_work, reset_default}};
        ...
        {
            rows.push_back(row);
            summary_csv << row.suite << ',' << row.case_name << ',' << row.logical_iterations << ','
                        << row.predicted_plan << ',' << row.measured_best_plan << ','
                        << (row.exact_winner ? 1 : 0) << ',' << (row.within_tolerance ? 1 : 0)
                        << ',' << row.prediction_confidence << ',' << row.predicted_runtime_ms
                        << ',' << row.measured_best_ms << ',' << row.prediction_error_percent << ','
                        << row.regret_percent << '\n';
            ...
            std::cout << "    predicted=" << std::setw(24) << row.predicted_plan
                      ...
                      << row.prediction_error_percent << '%' << " | regret=" << row.regret_percent
                      << "%\n";
        };
        ...
        {
            ++current;
            std::cout << '[' << current << '/' << total_cases << "] " << definition.name << "...\n";
            try
            {
        ...
        const std::array<std::pair<const char*, std::size_t>, 2> large_cases = {
            {{"large_record_8k", 8'192}, {"large_record_128k", 131'072}}};

        for (const auto& definition : large_cases)
        {
            ++current;
            std::cout << '[' << current << '/' << total_cases << "] " << definition.first
                      << "...\n";
            try
            {
                record(run_large_case(definition.first, definition.second, candidates_csv));
            }
            catch (const std::exception& exception)
            {
                ++failed;
        ...
        const std::array<std::tuple<const char*, std::size_t, std::size_t>, 2> pair_cases = {
            {{"pair_128x128", 128, 128}, {"pair_512x512", 512, 512}}};

        for (const auto& definition : pair_cases)
        {
            ++current;
            std::cout << '[' << current << '/' << total_cases << "] " << std::get<0>(definition)
                      << "...\n";
            try
            {
                record(run_pair_case(std::get<0>(definition),
        ...
        const double exact_accuracy = static_cast<double>(exact) / rows.size() * 100.0;
        const double near_accuracy = static_cast<double>(near_count) / rows.size() * 100.0;
        ...
        const double worst_regret = *std::max_element(regrets.begin(), regrets.end());

        std::ofstream metrics_csv(metrics_path);
        metrics_csv << "metric,value\n";
        metrics_csv << "cases," << rows.size() << '\n';
        metrics_csv << "failed_cases," << failed << '\n';
        metrics_csv << "exact_winner_accuracy_percent," << exact_accuracy << '\n';
        metrics_csv << "within_3_percent_accuracy_percent," << near_accuracy << '\n';
        metrics_csv << "mean_prediction_error_percent," << mean_error << '\n';
        metrics_csv << "median_prediction_error_percent," << median_error << '\n';
        metrics_csv << "p90_prediction_error_percent," << p90_error << '\n';
        ...
        const auto regret_summary = smart::validation::regret_metrics(regrets);
        ...
        std::cout << "Exact winner accuracy  : " << exact_accuracy << "%\n";
        std::cout << "Within 3% accuracy     : " << near_accuracy << "%\n";
        std::cout << "Median prediction error: " << median_error << "%\n";
        std::cout << "Median regret          : " << median_regret << "%\n";
        std::cout << "Worst regret           : " << worst_regret << "%\n";
        std::cout << "\nResults written to:\n" << output_dir.string() << '\n';
    ...
    {
        std::cerr << "Holdout validation failed: " << exception.what() << '\n';
        return 1;
    }
    catch (...)
    {
